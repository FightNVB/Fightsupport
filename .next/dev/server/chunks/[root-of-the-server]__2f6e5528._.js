...
"[project]/app/api/officials/weegstation/finalize/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function getFinalBoutStatus(row, evalStatus) {
...
async function POST(req) {
    try {
        ...
        const auth = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$weegstation$2f$routeAuth$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getWeegstationAuthContext"])(req, matchmakingId);
        const { admin } = auth;
        if (!canFinalize(auth)) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
        ...
        const { data: latestRow, error: latestErr } = await admin.from("weigh_in_bouts").select("controle_run_id, laatste_bewerking_op, updated_at, created_at").eq("matchmaking_id", matchmakingId).not("controle_run_id", "is", null).order("controle_run_id", {
            ascending: false
        }).order("laatste_bewerking_op", {
            ascending: false,
            nullsFirst: false
        }).order("updated_at", {
            ascending: false,
        ...
        const { data: rows, error: rowsErr } = await admin.from("weigh_in_bouts").select("*").eq("matchmaking_id", matchmakingId).eq("controle_run_id", activeControleRunId).order("partij_nr", {
            ascending: true
        });
        if (rowsErr) {
        ...
        const evaluatedRows = allRows.map((row)=>{
        ...
        const payload = evaluatedRows.filter(({ finalStatus })=>finalStatus === "OK").map(({ row, evalResult, finalStatus })=>({
        ...
        const { error: upsertErr } = await admin.from("definitive_matchmaking_bouts").upsert(payload, {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__2f6e5528._.js.map