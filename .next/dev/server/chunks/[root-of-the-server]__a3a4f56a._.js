...
"[project]/app/api/control-engine/admin-correct-bout/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function getLatestControleRunId(matchmaking_id) {
    const { data: ctxRows, error: ctxErr } = await supabase.from("controle_bout_context").select("controle_run_id, created_at").eq("matchmaking_id", matchmaking_id).order("created_at", {
        ascending: false
    }).limit(1);
    if (ctxErr) throw ctxErr;
    const fromCtx = ctxRows?.[0]?.controle_run_id ?? null;
    if (fromCtx) return String(fromCtx);
    const { data: resRows, error: resErr } = await supabase.from("controle_resultaten").select("controle_run_id, created_at").eq("matchmaking_id", matchmaking_id).order("created_at", {
        ascending: false
    }).limit(1);
...
async function getBoutRow(matchmaking_id, partij_nr) {
    const { data, error } = await supabase.from("matchmaking_bouts_raw").select("*").eq("matchmaking_id", matchmaking_id).eq("partij_nr", partij_nr).maybeSingle();
    if (error) throw error;
    return data ?? null;
}
async function getBoutContextRow(matchmaking_id, controle_run_id, partij_nr) {
    const { data, error } = await supabase.from("controle_bout_context").select("*").eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id).eq("partij_nr", partij_nr).maybeSingle();
    if (error) throw error;
    return data ?? null;
}
async function POST(req) {
    try {
        ...
        const oldVaRood = existingBout?.va_rood != null && String(existingBout.va_rood).trim() ? String(existingBout.va_rood).trim() : null;
        const oldVaBlauw = existingBout?.va_blauw != null && String(existingBout.va_blauw).trim() ? String(existingBout.va_blauw).trim() : null;
        const patch = {};
        ...
        if (roodVaChanged) {
            const prevExisting = existingBout?.rood_va_mm_prev != null && String(existingBout.rood_va_mm_prev).trim() ? String(existingBout.rood_va_mm_prev).trim() : null;
        ...
        if (blauwVaChanged) {
            const prevExisting = existingBout?.blauw_va_mm_prev != null && String(existingBout.blauw_va_mm_prev).trim() ? String(existingBout.blauw_va_mm_prev).trim() : null;
        ...
        const scopedBoutId = unwrapUuid(ctxAfterBuild?.bout_id) ?? unwrapUuid(existingBout?.bout_uid) ?? unwrapUuid(existingBout?.bout_id) ?? null;
        ...
        if (ctxRows.length === 0) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                ok: true,
                message: "Bout bijgewerkt, maar geen controle_bout_context gevonden voor deze partij.",
                matchmaking_id,
                partij_nr,
                controle_run_id
            });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a3a4f56a._.js.map