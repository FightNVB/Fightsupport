...
async function persistVaChangeInBoutContext(opts) {
    ...
    if (!ctx) return;
    const oldR = toVaStrict(ctx?.rood_va_mm) ?? toVaStrict(ctx?.va_rood) ?? toVaStrict(ctx?.rood_va) ?? null;
    const oldB = toVaStrict(ctx?.blauw_va_mm) ?? toVaStrict(ctx?.va_blauw) ?? toVaStrict(ctx?.blauw_va) ?? null;
    const changedR = !!newVaRood && newVaRood !== oldR;
    const changedB = !!newVaBlauw && newVaBlauw !== oldB;
    if (!changedR && !changedB) return;
    const patch = {};
...
async function POST(req) {
    ...
    try {
        ...
        const fallbackBoutId = unwrapUuid(bout?.bout_uid) ?? unwrapUuid(bout?.bout_id) ?? null;
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0ebecb6f._.js.map