...
"[project]/app/api/control-engine/bout-rescrape/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function runNodeScript(scriptPath, args, cwd) {
    return new Promise((resolve, reject)=>{
        ...
        const proc = (0, __TURBOPACK__imported__module__$5b$externals$5d2f$child_process__$5b$external$5d$__$28$child_process$2c$__cjs$29$__["spawn"])(nodeBin, [
            scriptPath,
            ...args
        ], {
            stdio: "inherit",
            shell: false,
            cwd: cwd ?? __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].dirname(scriptPath),
            windowsHide: true,
            env: {
                ...process.env,
        ...
        proc.on("close", (code)=>{
            if (code === 0) resolve();
            else reject(new Error(`Script failed: ${__TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].basename(scriptPath)} (exit code ${code})`));
        });
    });
}
async function getLatestControleRunId(matchmaking_id) {
    const { data, error } = await supabase.from("controle_runs").select("id, gestart_op").eq("matchmaking_id", matchmaking_id).order("gestart_op", {
        ascending: false
    }).limit(1);
    if (error) throw error;
    return data?.[0]?.id ?? null;
}
async function getBoutRow(matchmaking_id, partij_nr) {
    if (!Number.isFinite(partij_nr)) return null;
    const { data, error } = await supabase.from("matchmaking_bouts_raw").select("*").eq("matchmaking_id", matchmaking_id).eq("partij_nr", partij_nr).limit(1);
    if (error) throw error;
    return data?.[0] ?? null;
}
/**
 ...
 */ async function persistVaChangeInBoutRaw(opts) {
    ...
    if (Object.keys(prevPatch).length > 0) {
        const { error: prevErr } = await supabase.from("matchmaking_bouts_raw").update(prevPatch).eq("matchmaking_id", matchmaking_id).eq("partij_nr", partij_nr);
        if (prevErr) {
    ...
    const { error: upErr } = await supabase.from("matchmaking_bouts_raw").update(patch).eq("matchmaking_id", matchmaking_id).eq("partij_nr", partij_nr);
...
async function getBoutContextRow(matchmaking_id, controle_run_id, partij_nr) {
    if (!Number.isFinite(partij_nr)) return null;
    const { data, error } = await supabase.from("controle_bout_context").select("*").eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id).eq("partij_nr", partij_nr).limit(1);
    if (error) throw error;
    return data?.[0] ?? null;
}
/**
 ...
 */ async function persistVaChangeInBoutContext(opts) {
    ...
    const oldR = toVaStrict(ctx?.va_rood) ?? toVaStrict(ctx?.rood_va) ?? null;
    const oldB = toVaStrict(ctx?.va_blauw) ?? toVaStrict(ctx?.blauw_va) ?? null;
    const changedR = !!newVaRood && newVaRood !== oldR;
    const changedB = !!newVaBlauw && newVaBlauw !== oldB;
    if (!changedR && !changedB) return;
    const patch = {};
    ...
    try {
        const { error } = await supabase.from("controle_bout_context").update(patch).eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id).eq("partij_nr", partij_nr);
        if (error) throw error;
...
async function POST(req) {
    ...
    try {
        ...
        const partij_nr = body?.partij_nr != null && Number.isFinite(Number(body.partij_nr)) ? Number(body.partij_nr) : undefined;
        ...
        console.log("[bout-rescrape] start", {
            matchmaking_id,
            controle_run_id,
        ...
        const vaRood = va_rood_in ?? toVaStrict(bout?.va_rood) ?? toVaStrict(bout?.rood_va) ?? toVaStrict(bout?.rood_va_mm) ?? null;
        const vaBlauw = va_blauw_in ?? toVaStrict(bout?.va_blauw) ?? toVaStrict(bout?.blauw_va) ?? toVaStrict(bout?.blauw_va_mm) ?? null;
        const vaList = [
            vaRood,
            vaBlauw
        ].filter(Boolean);
        ...
        if (vaList.length === 0) {
            ...
            const { data: ctxRows, error: cErr } = await supabase.from("controle_bout_context").select("*").eq("controle_run_id", controle_run_id).eq("matchmaking_id", matchmaking_id).order("partij_nr", {
                ascending: true
            });
            if (cErr) throw cErr;
            await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$rulesEngine$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["rulesEngine"])({
                matchmaking_id,
                controle_run_id,
                ctxRows: ctxRows ?? []
            });
        ...
        console.log("[bout-rescrape] ▶ bundle scrape", {
            vaList
        });
        ...
        await runNodeScript(bundlePath, [
            ...
            controle_run_id,
            ...vaList
        ], __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].dirname(bundlePath));
        ...
        // ✅ rulesEngine over ALLE ctxRows (anders verdwijnen andere partijen)
        const { data: ctxRows, error: ctxErr } = await supabase.from("controle_bout_context").select("*").eq("controle_run_id", controle_run_id).eq("matchmaking_id", matchmaking_id).order("partij_nr", {
            ascending: true
        });
        if (ctxErr) throw ctxErr;
        console.log("[bout-rescrape] ▶ rulesEngine (ctxRows full)", {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__6516f74f._.js.map