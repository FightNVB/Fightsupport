...
const LOCK_FILE = path.join(
  __dirname,
  ".fp_officials_queue_worker.lock",
);
...
function acquireLock() {
  try {
    ...
    if (processExists(existingPid)) {
      console.log(
        `[fp-officials-queue] worker draait al met PID ${existingPid}; deze launcher stopt.`,
      );
      return false;
    }
...
function runOfficialsBatch(batch) {
  return new Promise((resolve, reject) => {
    console.log(
      `[fp-officials-queue] ▶ batch ${batch.matchmaking_id} / ${batch.controle_run_id}: ${batch.va_numbers.length} VA's`,
    );
      ...
      ],
      {
        ...
        env: {
          ...
          PUPPETEER_HEADLESS:
            ...
            "false",
          FP_OFFICIALS_WORKERS:
            process.env.FP_OFFICIALS_WORKERS ??
            process.env.WORKERS ??
            "8",
          WORKERS:
            process.env.FP_OFFICIALS_WORKERS ??
            ...
            "8",
          STAGGER_MS:
            process.env.STAGGER_MS ?? "250",
          TAB_ATTEMPTS:
            process.env.TAB_ATTEMPTS ?? "5",
          SOFT_WAIT_MS:
            process.env.SOFT_WAIT_MS ?? "200",
          BETWEEN_ATTEMPTS_MS:
            process.env.BETWEEN_ATTEMPTS_MS ?? "350",
          FP_OFFICIALS_TIMEOUT_MS:
            process.env.FP_OFFICIALS_TIMEOUT_MS ??
            "45000",
        },
      },
    );
    ...
    child.stdout?.on("data", (data) => {
      process.stdout.write(
        `[fp-officials batch] ${data.toString()}`,
      );
    });
    ...
    child.on("close", (code) => {
      if (code === 0) {
        ...
        reject(
          new Error(
            `Officials scraper stopte met exit code ${code}`,
          ),
        );
      }
    });
...
async function processBatch(batch) {
  try {
    ...
    console.log(
      `[fp-officials-queue] ✅ batch klaar: ${batch.rows.length} items`,
    );
  } catch (error) {
    ...
    console.error(
      `[fp-officials-queue] ❌ batch ${batch.matchmaking_id}: ${message}`,
    );
  }
}
...
async function main() {
  ...
  console.log(
    `[fp-officials-queue] 🧵 queue-worker actief PID ${process.pid}`,
  );
  ...
  try {
    ...
    console.log(
      "[fp-officials-queue] 🏁 queue leeg; worker afgesloten",
    );
  }
}
...
main().catch((error) => {
  console.error(
    "[fp-officials-queue] fatale fout:",
    error,
  );
  releaseLock();
  process.exit(1);
