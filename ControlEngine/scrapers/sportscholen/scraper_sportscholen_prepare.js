// ControlEngine/scrapers/sportscholen/scraper_sportscholen_prepare.js
// Fase 1: download volledige FightPassport sportscholen-Excel en update basislijst.
...
async function downloadExcel(page, browser) {
  console.log("📥 Sportscholen Excel downloaden…");

  const downloadDir = path.resolve(
    __dirname,
    "downloads",
    `prepare_${crypto.randomUUID().slice(0, 8)}`
  );
  ...
  try {
    ...
    if (btnImg) await page.click(excelSelImg);
    else await page.click(excelSelDiv);

    const file = await waitForNewExcel(downloadDir, 60000);
    return { file, downloadDir };
  } catch (error) {
    cleanupDownloadDirExact(downloadDir);
    throw error;
  } finally {
...
function cleanupDownloadDirExact(dir) {
  if (!dir) return;
  try {
...
export async function scraperSportscholen() {
  ...
  try {
    ...
    const downloaded = await downloadExcel(page, browser);
    try {
      const parsed = await parseExcel(downloaded.file);
      await saveToSupabase(parsed);
      console.log("🎉 Sportscholen scrape compleet!");
    } finally {
      cleanupDownloadDirExact(downloaded.downloadDir);
    }
  } catch (err) {
