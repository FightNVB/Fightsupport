...
function isRelevantHit(h: AnyRow) {
  const r=hitResult(h);
  const code=s(h.rule_code ?? h.rule).toUpperCase();

  if (!s(h.boodschap)) return false;
  if (r === "OK") return false;

  // Trainers zien normaal geen INFO-resultaten.
  // Enige uitzondering: dopingcertificaat, omdat deze tijdelijke
  // overgangsmelding expliciet aan de trainer van de betreffende vechter
  // getoond moet worden.
  if (r === "INFO") {
    return code.includes("DOPINGCERTIFICAAT");
  }
