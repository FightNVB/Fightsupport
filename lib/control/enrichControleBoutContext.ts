...
// ✅ Admin-eigen verrijking van controle_bout_context met KEURMERK
...
function getKeurmerkEventStatus(school: any, eventDate: any) {
  const eventIso = toIsoDateOnly(eventDate);
  let rawPeriods: any = school?.keurmerk_periodes ?? [];
  if (typeof rawPeriods === "string") {
    try { rawPeriods = JSON.parse(rawPeriods); } catch { rawPeriods = []; }
  }
  if (!Array.isArray(rawPeriods)) rawPeriods = [];
  const periods: { start: string | null; einde: string | null }[] = [];
  const pushPeriod = (startValue: any, endValue: any) => {
    const start = toIsoDateOnly(startValue);
    const einde = toIsoDateOnly(endValue);
    if (!start && !einde) return;
    if (periods.some((p) => p.start === start && p.einde === einde)) return;
    periods.push({ start, einde });
  };
  for (const p of rawPeriods) {
    pushPeriod(p?.start ?? p?.keurmerk_start ?? p?.startdatum ?? p?.start_date, p?.einde ?? p?.keurmerk_einde ?? p?.einddatum ?? p?.end_date);
  }
  pushPeriod(school?.keurmerk_start, school?.keurmerk_einde);
  pushPeriod(school?.keurmerk_volgende_start, school?.keurmerk_volgende_einde);
  if (periods.length === 0) {
    pushPeriod(null, school?.keurmerk_eind ?? school?.keurmerk_einde ?? school?.einde_keurmerk ?? school?.keurmerk_laatste_einde);
  }
  const covering = eventIso ? periods.find((p) => (!p.start || p.start <= eventIso) && (!p.einde || p.einde >= eventIso)) ?? null : null;
  const latestEnd = [
    ...periods.map((p) => p.einde),
    toIsoDateOnly(school?.keurmerk_laatste_einde),
    toIsoDateOnly(school?.keurmerk_volgende_einde),
    toIsoDateOnly(school?.keurmerk_einde),
    toIsoDateOnly(school?.keurmerk_eind),
    toIsoDateOnly(school?.einde_keurmerk),
  ].filter(Boolean).sort().at(-1) ?? null;
  return { geldig: !!covering, start: covering?.start ?? null, einde: covering?.einde ?? null, latestEnd };
}
...
async function fetchAllSportschoolAliases() {
  ...
  while (true) {
    const to = from + pageSize - 1;
    const { data, error } = await supabaseAdmin.from("sportschool_aliases").select("alias_text, sportschool_id").range(from, to);
    if (error) throw error;
    const chunk = data ?? [];
    all.push(...chunk);
    if (chunk.length < pageSize) break;
...
function tokenSet(s: string) { return new Set(String(s ?? "").split(" ").filter(Boolean)); }
...
type GymMatch = { row: any | null; reason: string | null };

function findSportschoolBySportschoolId(list: any[], sid: any) {
  const s = String(sid ?? "").trim();
  if (!s) return null;
  return list.find((x) => String(x?.sportschool_id ?? "").trim() === s) ?? list.find((x) => String(x?.id ?? "").trim() === s) ?? null;
}
...
function findExactNameOrAliasFallback(sportscholen: any[], gymNaam: string, aliasMaps?: AliasMaps): GymMatch {
  ...
  const strictCompact = compactStrictName(raw);
  const loose = norm(raw);
  const looseCompact = compactNorm(loose);
  const candidates: any[] = [];
  const seen = new Set<string>();
  const add = (row: any) => {
    if (!row) return;
    const id = String(row?.sportschool_id ?? "").trim() || `${normStrictName(row?.naam)}|${String(row?.plaats ?? row?.stad ?? "").trim()}|${String(row?.land ?? row?.country ?? "").trim()}`;
  ...
  for (const row of sportscholen ?? []) {
    ...
    if (!naam) continue;
    if (normStrictName(naam) === strict || compactStrictName(naam) === strictCompact || norm(naam) === loose || compactNorm(norm(naam)) === looseCompact) add(row);
  }
  for (const alias of aliasMaps?.aliasRows ?? []) {
    const aliasRaw = String(alias?.alias_text ?? "").trim();
    if (!aliasRaw) continue;
    if (normStrictName(aliasRaw) === strict || compactStrictName(aliasRaw) === strictCompact || norm(aliasRaw) === loose || compactNorm(norm(aliasRaw)) === looseCompact) {
...
function stripCountryHintsFromRaw(raw: string) {
  ...
  s = s.replace(/\((Nederland|België|Belgie|Duitsland|Deutschland|Germany|Frankrijk|France|Spanje|Spain|Espana|España|Denmark|Danmark|Denemarken|United Kingdom|Verenigd Koninkrijk|Engeland|England|Turkije|Turkey|Türkiye|Turkiye)\)/gi, " ");
  s = s.replace(/\b(NL|AT|BE|BG|HR|CY|CZ|DK|EE|FI|FR|DE|GR|HU|IE|IT|LV|LT|LU|MT|PL|PT|RO|SK|SI|ES|SE|UK|GB|ENG|TR)\b/gi, " ");
  s = s.replace(/\b(Nederland|België|Belgie|Duitsland|Deutschland|Germany|Frankrijk|France|Spanje|Spain|Espana|España|Denmark|Danmark|Denemarken|United Kingdom|Verenigd Koninkrijk|Engeland|England|Turkije|Turkey|Türkiye|Turkiye)\b/gi, " ");
...
function stripKnownPlaceSuffixes(raw: string, knownPlaces: string[]) {
  ...
  while (changed) {
    ...
    for (const place of knownPlaces) {
      if (!place) continue;
      const patterns = [new RegExp(`\\b${escapeRegex(place)}\\b$`, "i"), new RegExp(`[\\-/,]\\s*${escapeRegex(place)}$`, "i")];
...
function buildAliasLookupVariants(rawGym: string, knownPlaces: string[]) {
  ...
  addVariant(raw); addVariant(noCountry); addVariant(noPlace);
  for (const part of noPlace.split(/[\/|,-]/g).map((x) => x.trim()).filter(Boolean)) addVariant(part);
  const words = noPlace.split(/\s+/).filter(Boolean);
  for (let i = words.length; i >= 2; i--) addVariant(words.slice(0, i).join(" "));
  return out;
}
function tryAliasMatch(sportscholen: any[], gymNaam: string, aliasMaps: AliasMaps | undefined, knownPlaces: string[]): GymMatch | null {
  ...
  for (const key of variants) {
    if (key.startsWith("__compact__:")) {
      const sid = aliasMaps.aliasCompactToId.get(key.replace("__compact__:", ""));
      if (sid) {
...
function scoreCandidate(x: any, g: string, gRaw: string, key: string, landHint: LandHint | null) {
  ...
  let score = ov * 0.72 + (1 - Math.min(1, d / len)) * 0.28;
  const plaats = x?.plaats ?? x?.stad ?? "";
  if (hasPlaatsHint(gRaw, plaats)) score += 0.22;
  if (landMatchesHint(x?.land ?? x?.country ?? null, landHint)) score += 0.18;
...
function chooseBestFromCandidates(candidates: any[], g: string, gRaw: string, key: string, landHint: LandHint | null): GymMatch {
  ...
  let best: any = null, bestScore = -1, secondScore = -1;
  ...
  if (best && bestScore >= 0.72 && bestScore - secondScore >= 0.04) return { row: best, reason: null };
  return { row: null, reason: "Meerdere matches (ambigue) — maak alias aan." };
}

function looseGymNameCompatible(aRaw: string, bRaw: string) {
  const aStrict = compactStrictName(aRaw);
  const bStrict = compactStrictName(bRaw);
  if (aStrict && bStrict && aStrict === bStrict) return true;
...
async function fetchLatestResultGymsByVa(vaNummers: string[]) {
  ...
  const { data, error } = await supabaseAdmin.from("fightpassport_results").select("id,va_nummer,datum,sportschool,last_seen_at").in("va_nummer", unique).order("datum", { ascending: false, nullsFirst: false }).order("id", { ascending: false });
  if (error) throw error;
...
function mmNameMatchesSchoolOrAlias(gymNaam: string, school: any, aliasMaps: AliasMaps) {
...
async function fetchSchoolFighterLinksByVa(vaNummers: string[]) {
  ...
  if (unique.length === 0) return map;
  const { data, error } = await supabaseAdmin.from("fightpassport_school_fighters").select("va_nummer,sportschool_id,actief,last_seen_at").in("va_nummer", unique).order("last_seen_at", { ascending: false });
  if (error) throw error;
...
function findGymMatchFromVaLinksOnly(opts: { sportscholen: any[]; gymNaam: string; vaNummer: string; schoolLinksByVa: Map<string, SchoolFighterLink[]>; aliasMaps: AliasMaps }): GymMatch {
  ...
  if (candidates.length === 0) return { row: null, reason: "Geen gekoppelde sportschool voor deze VA." };
  ...
  return { row: null, reason: restricted.reason ?? "De gekoppelde sportschool van deze VA komt niet overeen met de sportschool uit de matchmaking." };
}
...
function findGymMatch(sportscholen: any[], gymNaam: string, aliasMaps?: AliasMaps): GymMatch {
  ...
  if (strippedStrict && strippedStrict !== rawStrict) {
    ...
    if (strictBaseHits.length > 0) return chooseBestFromCandidates(strictBaseHits, strippedNorm || strippedStrict, gRaw, key, landHint);
    const strictBaseCompactHits = list.filter((x) => compactStrictName(x?.naam) === strippedCompactStrict);
    if (strictBaseCompactHits.length > 0) return chooseBestFromCandidates(strictBaseCompactHits, strippedNorm || strippedStrict, gRaw, key, landHint);
  }
  if (strippedNorm && strippedNorm !== g) {
    const exactBaseHits = list.filter((x) => norm(x?.naam) === strippedNorm);
    if (exactBaseHits.length > 0) return chooseBestFromCandidates(exactBaseHits, strippedNorm, gRaw, key, landHint);
    const compactBaseHits = list.filter((x) => compactNorm(norm(x?.naam)) === strippedCompact);
    if (compactBaseHits.length > 0) return chooseBestFromCandidates(compactBaseHits, strippedNorm, gRaw, key, landHint);
  }
  const subsetHits = list.filter((x) => {
    ...
    if (!ok) return false;
    const tokCount = Math.max(tokenSet(g).size, strippedNorm ? tokenSet(strippedNorm).size : 0);
    if (tokCount >= 2) {
  ...
  let best: any = null, bestScore = -1, bestSecond: any = null, bestSecondScore = -1;
  const scoreBase = strippedNorm || g;
  for (const x of list) {
    const score = scoreCandidate(x, scoreBase, gRaw, key, landHint);
    if (score < 0) continue;
    if (score > bestScore) { bestSecond = best; bestSecondScore = bestScore; best = x; bestScore = score; }
    else if (score > bestSecondScore) { bestSecond = x; bestSecondScore = score; }
  }
...
function unwrapUuid(v: any): string | null {
  ...
  if (typeof v === "object") {
    const cand = (typeof (v as any).id === "string" && (v as any).id) || (typeof (v as any).bout_id === "string" && (v as any).bout_id) || (typeof (v as any).bout_uid === "string" && (v as any).bout_uid) || null;
    return cand ? String(cand).trim() : null;
  }
  return null;
}

function buildKeurmerkPatchForGym(opts: { gym: string; evenement_datum?: string | null; sportscholen: any[]; aliasMaps: AliasMaps; valueKey: string; reasonKey: string }) {
  ...
  let match = gymValue ? findGymMatch(sportscholen, gymValue, aliasMaps) : { row: null, reason: null };
  if (gymValue && !hint && (!match.row || isForeignNonNL(match.row?.land ?? match.row?.country))) {
    const exactFallback = findExactNameOrAliasFallback(sportscholen, gymValue, aliasMaps);
    if (exactFallback.row) match = exactFallback;
    else if (match.row && isForeignNonNL(match.row?.land ?? match.row?.country)) match = exactFallback;
  }
  const found = match.row;
  if (!found) {
    if (isForeignHint(hint)) {
      patch[valueKey] = true;
      patch[reasonKey] = buildForeignKeurmerkReason({ gym: gymValue, land: landHintToLabel(hint) });
      return patch;
    }
    patch[valueKey] = null;
  ...
  patch[reasonKey] = keurmerkStatus.geldig ? `${matchInfo}\nKeurmerk geldig t/m ${keurmerkStatus.einde ?? "-"}.` : `${matchInfo}\nGeen geldig keurmerk op eventdatum. Laatste bekende keurmerk eindigt/eindigde op ${keurmerkStatus.latestEnd ?? "-"}.`;
  return patch;
}

export async function enrichControleBoutContext(matchmaking_id: string, controle_run_id: string, opts?: { partij_nr?: number | null; bout_id?: string | null }) {
  ...
  console.log("[enrichControleBoutContext] scope", { matchmaking_id, controle_run_id, partij_nr: scopedPartijNr, bout_id: scopedBoutId, rows: ctxRows.length });
  for (const row of ctxRows) {
    ...
    const roodHint = detectLandHintFromGymText(roodGym), blauwHint = detectLandHintFromGymText(blauwGym);
    if (!rood) {
      ...
      const land = landLabelForMatch(landDb, roodHint);
      const keurmerkStatus = getKeurmerkEventStatus(rood, (row as any)?.evenement_datum);
      const matchInfo = `${mmLine(roodGym)}\n↳ gematcht met "${rood.naam}" (${rood.plaats ?? rood.stad ?? "?"}, ${land ?? "?"})`;
      const isForeign = landDb ? isForeignNonNL(landDb) : isForeignHint(roodHint);
      if (isForeign) {
        patch.keurmerk_rood = true;
        patch.keurmerk_reden_rood = buildForeignKeurmerkReason({ gym: roodGym, land: land ?? (landDb ? "Buitenland" : landHintToLabel(roodHint)), matchInfo });
      } else {
        patch.keurmerk_rood = keurmerkStatus.geldig;
        patch.keurmerk_reden_rood = keurmerkStatus.geldig ? `${matchInfo}\nKeurmerk geldig t/m ${keurmerkStatus.einde ?? "-"}.` : `${matchInfo}\nGeen geldig keurmerk op eventdatum. Laatste bekende keurmerk eindigt/eindigde op ${keurmerkStatus.latestEnd ?? "-"}.`;
      }
    }
    if (!blauw) {
      ...
      const land = landLabelForMatch(landDb, blauwHint);
      const keurmerkStatus = getKeurmerkEventStatus(blauw, (row as any)?.evenement_datum);
      const matchInfo = `${mmLine(blauwGym)}\n↳ gematcht met "${blauw.naam}" (${blauw.plaats ?? blauw.stad ?? "?"}, ${land ?? "?"})`;
      const isForeign = landDb ? isForeignNonNL(landDb) : isForeignHint(blauwHint);
      if (isForeign) {
        patch.keurmerk_blauw = true;
        patch.keurmerk_reden_blauw = buildForeignKeurmerkReason({ gym: blauwGym, land: land ?? (landDb ? "Buitenland" : landHintToLabel(blauwHint)), matchInfo });
      } else {
        patch.keurmerk_blauw = keurmerkStatus.geldig;
        patch.keurmerk_reden_blauw = keurmerkStatus.geldig ? `${matchInfo}\nKeurmerk geldig t/m ${keurmerkStatus.einde ?? "-"}.` : `${matchInfo}\nGeen geldig keurmerk op eventdatum. Laatste bekende keurmerk eindigt/eindigde op ${keurmerkStatus.latestEnd ?? "-"}.`;
      }
    }
    const { error: uErr } = await supabaseAdmin.from("controle_bout_context").update(patch).eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id).eq("bout_id", bout_id);
    if (uErr) throw uErr;
  }
  let tCtxQ = supabaseAdmin.from("controle_toernooi_context").select("id, fighter_id, va_nummer, sportschool_mm, sportschool, evenement_datum").eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id);
  ...
  for (const row of tournamentRows ?? []) {
    ...
    if (tournamentMatch.row) {
      ...
      const keurmerkStatus = getKeurmerkEventStatus(found, (row as any).evenement_datum);
      const matchInfo = `↳ [MM sportschool:] "${gym || "-"}"\n↳ gematcht met "${found.naam}" (${found.plaats ?? found.stad ?? "?"}, ${land ?? "?"})`;
      const isForeign = landDb ? isForeignNonNL(landDb) : isForeignHint(hint);
      if (isForeign) patch = { heeft_keurmerk: true, keurmerk_reason: buildForeignKeurmerkReason({ gym, land: land ?? "Buitenland", matchInfo }) };
      else patch = { heeft_keurmerk: keurmerkStatus.geldig, keurmerk_reason: keurmerkStatus.geldig ? `${matchInfo}\nKeurmerk geldig t/m ${keurmerkStatus.einde ?? "-"}.` : `${matchInfo}\nGeen geldig keurmerk op eventdatum. Laatste bekende keurmerk eindigt/eindigde op ${keurmerkStatus.latestEnd ?? "-"}.` };
    } else {
      patch = buildKeurmerkPatchForGym({ gym, evenement_datum: String((row as any).evenement_datum ?? "").trim() || null, sportscholen, aliasMaps, valueKey: "heeft_keurmerk", reasonKey: "keurmerk_reason" });
    }
    const { error: tuErr } = await supabaseAdmin.from("controle_toernooi_context").update(patch).eq("matchmaking_id", matchmaking_id).eq("controle_run_id", controle_run_id).eq("id", rowId);
