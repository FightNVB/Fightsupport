 ...
 */
function parseDenseFighterText(text: string, senderName: string, sourceType: string) {
  ...
  const globalSchool = s(compact.match(/\b(?:van|namens)\s+(?:sportschool|gym)\s+([^,.;]+)/i)?.[1]) || "";
  ...
  for (let index = 0; index < dates.length; index++) {
    ...
    drafts.push({
      ...
      trainer_school: parseSchool(enrichedBlock) || globalSchool,
...
export async function POST(req: Request) {
  try {
    ...
    const denseParsed = parseDenseFighterText(text, senderName, sourceType);
    ...
    const missingVaNames = [...new Set(
      parsed.drafts
        .filter((draft: any) => !s(draft.source_va).replace(/\D/g, ""))
        .map((draft: any) => s(draft.source_name))
        .filter(Boolean),
    )];

    const nameLookups = await Promise.all(
      missingVaNames.map(async (name) => {
    ...
    const rows = parsed.drafts.map((draft: any) => {
      const normalizedVa = s(draft.source_va)
        .replace(/\D/g, "")
        .replace(/^0+(?=\d)/, "");
      const exactNameMatches = !normalizedVa
        ? (byName.get(normName(draft.source_name)) ?? []).filter(
            (candidate: any) => normName(candidate.naam) === normName(draft.source_name),
          )
        : [];
      const fighter: any = normalizedVa
        ? byVa.get(normalizedVa)
        : exactNameMatches.length === 1
          ? exactNameMatches[0]
          : null;
      const resolvedVa = normalizedVa || (fighter
        ? String(fighter.va_nummer).replace(/\D/g, "").replace(/^0+(?=\d)/, "")
        : "");
