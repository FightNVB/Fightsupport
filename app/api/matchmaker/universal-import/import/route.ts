...
export async function POST(req: Request) {
  try {
    ...
    for (const row of rows) {
      ...
      const trainerSchool = s(row.trainer_school) || fallbackSchool;
      if (!fighter || !weight || !trainerSchool || !s(row.discipline) || !s(row.klasse) || maxWeight == null) { skipped.push({ va_nummer: va, reason: "Verplichte koppeling of matchmakerkeuze ontbreekt" }); continue; }
      ...
      inserts.push({
        ...
        raw: {
          ...
          bron_opgegeven: {
            ...
            sportschool: trainerSchool, email: s(row.source_email) || null,
          },
