...
export async function GET(req:NextRequest){
 try{
  ...
  const gyms=data.gyms.map(g=>{ const link=(links??[]).find((l:any)=>l.sportschool_key===g.key); const own=link?responses.filter((r:any)=>r.link_id===link.id):[]; const counts={open:0,akkoord:0,afgewezen:0,bespreken:0}; for(const b of g.bouts){ const r=own.find((x:any)=>x.bout_id===b.id); if(!r)counts.open++; else if(r.status==="akkoord")counts.akkoord++; else if(r.status==="afgewezen")counts.afgewezen++; else if(r.status==="bespreken")counts.bespreken++; }
   return {...g,link:link?{id:link.id,token:link.token,isEnabled:link.is_enabled,openedAt:link.opened_at,createdAt:link.created_at,expiresAt:link.expires_at}:null,counts}; });
...
export async function POST(req:NextRequest){
 try{
  ...
  if(existing){ if(existing.is_enabled===false || (existing.expires_at && new Date(existing.expires_at).getTime()<=Date.now())){ const now=new Date(); const expiresAt=new Date(now.getTime()+7*24*60*60*1000).toISOString(); const up=await supabaseAdmin.from("trainer_match_links").update({is_enabled:true,sportschool_naam:gym.naam,token:token(),opened_at:null,expires_at:expiresAt,updated_at:now.toISOString()}).eq("id",existing.id).select("*").single(); if(up.error)throw up.error; return NextResponse.json({ok:true,link:up.data}); } return NextResponse.json({ok:true,link:existing}); }
...
export async function PATCH(req:NextRequest){
 try{ await auth(req); const body=await req.json().catch(()=>({})); const id=s(body.id); if(!id)return NextResponse.json({error:"id ontbreekt"},{status:400}); const patch:any={updated_at:new Date().toISOString()}; if(typeof body.isEnabled==="boolean")patch.is_enabled=body.isEnabled; if(body.rotate===true){patch.token=token();patch.opened_at=null;patch.expires_at=new Date(Date.now()+7*24*60*60*1000).toISOString();} const up=await supabaseAdmin.from("trainer_match_links").update(patch).eq("id",id).select("*").single(); if(up.error)throw up.error; return NextResponse.json({ok:true,link:up.data}); }
