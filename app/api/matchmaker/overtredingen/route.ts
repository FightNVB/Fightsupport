...
function normalize(row: any, profileById: Map<string, any>) {
  ...
  const bron = row.bron_type || row.melding_bron || row.bron || noteBron || (String(note).toLowerCase().includes("matchmaker") ? "matchmaker" : null);
...
async function getLoggedInProfile(supabase: any, req: NextRequest) {
  ...
  if (!token) {
    return { profile: null, error: "Geen ingelogde matchmaker gevonden. Log opnieuw in." };
  }
  ...
  if (!profile?.bondteam) {
    return { profile: null, error: "Geen bondteam gekoppeld aan deze matchmaker." };
  }
