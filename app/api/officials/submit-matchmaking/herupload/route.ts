// app/api/officials/submit-matchmaking/herupload/route.ts
...
export async function POST(req: Request) {
  try {
    ...
    let role = roleLower((auth as any)?.role ?? profileForUpload.role);
    ...
    if (role === "official" || role === "hoofdofficial") {
      ...
      const normalizedUserBond = normalizeBondteamValue(userBond);
      if (normalizedUserBond !== normalizedBondteam) {
        return bad(
    ...
    if (incomingMatchmakingId) {
      const { data: existingMatchmaking, error: existingMatchmakingErr } =
        await supabaseAdmin
          .from("matchmakings")
          .select("id,event_id,matchmaker_id,maker_type,maker_user_id,bron_type,status,stadium,final_status,huidige_eigenaar_type,huidige_eigenaar_user_id,huidige_eigenaar_bondteam,bondteam")
      ...
      const existingBron = String(existingMatchmakingForReuse?.bron_type ?? "").trim().toLowerCase();
      const existingMakerUserId = String(existingMatchmakingForReuse?.maker_user_id ?? "").trim();
      const existingOwnerBondteam = normalizeBondteamValue(
        existingMatchmakingForReuse?.huidige_eigenaar_bondteam ??
          existingMatchmakingForReuse?.bondteam
      );

      if (
        (role === "official" || role === "hoofdofficial") &&
        (existingBron !== "official_upload" ||
          existingMakerUserId !== userId ||
          existingOwnerBondteam !== normalizedBondteam)
      ) {
        return bad("Je mag alleen je eigen official-upload heruploaden.", 403);
      }

      // Official herupload blijft altijd official_upload.
      // Niet de oude waarde bewaren als een eerdere route hem fout als admin/matchmaker heeft gezet.
    ...
    // Bij official-herupload is de uploader de maker, niet de matchmaker.
    // Daarom nooit matchmaker_id vullen en geen oude foutieve admin/matchmaker waarden bewaren.
    const makerType = role === "hoofdofficial" ? "hoofdofficial" : "official";
...
const existingStage =
    ...
    // Herupload door official blijft bij het eigen bondteam.
    // De official is maker/uploader, maar niet de huidige eigenaar.
    ...
    await ensureLifecycleRecord({
      ...
      actorRole: role,
      metadata: {
        route: "app/api/officials/submit-matchmaking/herupload/route.ts",
        auth_user_id: authUserId,
        preserved_bron_type: lifecycleBronType,
        official_herupload_owner_is_bondteam: true,
      },
    });
