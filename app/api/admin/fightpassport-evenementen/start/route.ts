...
export async function GET(req: Request) {
  try {
    ...
    const eventRows = events.map((event) => {
      ...
      // FightPassport zelf is leidend voor het aantal echte partijen.
      // UITSLAGEN -> "Aantal" is het aantal partijen dat daadwerkelijk bij het event
      // geregistreerd staat. Als dat veld bij oude, nog niet opnieuw gescrapete events
      // nog null is, gebruiken we MATCHMAKING -> "Aantal partijen". Alleen als beide
      // ontbreken gebruiken we de resultaten-deduplicatie als tijdelijke fallback.
      ...
      const fallbackUitResultaten = eventFightKeys.get(key)?.size ?? 0;

      const partijen =
        Number.isFinite(uitslagenAantal) && uitslagenAantal !== null
          ? uitslagenAantal
          : Number.isFinite(matchmakingAantalPartijen) && matchmakingAantalPartijen !== null
            ? matchmakingAantalPartijen
            : fallbackUitResultaten;
    ...
    return NextResponse.json(
      {
        ...
        totals: {
          ...
          partijen: eventRows.reduce((sum, event) => sum + Number(event.partijen ?? 0), 0),
