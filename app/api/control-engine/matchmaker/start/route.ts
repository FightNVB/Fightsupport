...
const SCRAPER_FILE = "scraper_fp_bundle_MM.js";
const LOG_PREFIX = "fp_bundle_matchmaker";

function isAllowedRole(role: string | null | undefined) {
  return role === "matchmaker" || role === "superadmin";
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (Array.isArray(hits) && hits[0]) {
      dlog("[control-engine/matchmaker/start] hit_sample", hits[0]);
    }
