    ...
    >
      <div className="grid grid-cols-1 items-center gap-4 xl:grid-cols-[1fr_auto_1fr]">
        <div>
          ...
          <div className="mt-1 text-sm text-white/75">Promotor aanvraagformulier</div>
  ...
  return (
    <div className="rounded-[24px] p-5" style={LIGHT_PANEL}>
      <div className="mb-4 flex items-start gap-3">
        <div className="flex h-11 w-11 items-center justify-center rounded-[12px]" style={{
          background: "linear-gradient(180deg, #ff6b22 0%, #ff4d00 55%, #b93200 100%)",
          color: "#fff",
        }}>
          {icon}
        </div>
      ...
      <div className="mb-4 h-[4px] w-full rounded-full" style={{
        background: "linear-gradient(90deg, rgba(255,77,0,0.95) 0%, rgba(255,77,0,0.18) 48%, rgba(0,0,0,0.08) 100%)",
      }} />
      {children}
    </div>
  );
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="mb-1 text-[12px] font-extrabold uppercase tracking-[0.10em] text-slate-700">{label}</div>
      {children}
    </div>
  );
}
...
export default function PromotorEventRequestPage() {
  ...
  const [voorkeurHoofdofficialUserId, setVoorkeurHoofdofficialUserId] = useState("");
  ...
  useEffect(() => {
    async function loadOfficials() {
      try {
        const res = await authedFetch("/api/admin/event-requests/official-options", { method: "GET" });
        const json = await res.json().catch(() => ({}));
        if (res.ok) setOfficials(Array.isArray(json?.rows) ? json.rows : []);
      } catch {
        setOfficials([]);
      }
    }
    ...
    try {
      const res = await authedFetch("/api/promotor/event-requests/create", {
        ...
        body: JSON.stringify({
          ...
          disciplines,
          voorkeur_hoofdofficial_user_id: voorkeurHoofdofficialUserId || null,
  ...
  return (
    <Shell>
      <Header onBack={() => router.back()} onDashboard={() => router.push("/dashboard/promotor")} />

      <div className="px-4 py-6 md:px-6">
        <div className="rounded-[24px] p-5 md:p-6" style={{
          background: "linear-gradient(180deg, rgba(255,255,255,0.96) 0%, rgba(239,242,246,0.98) 100%)",
          border: "2px solid rgba(95,105,118,0.40)",
          boxShadow: "0 18px 36px rgba(0,0,0,0.14), inset 0 1px 0 rgba(255,255,255,0.88)",
        }}>
          <div className="text-center">
            <h1 className="text-4xl font-extrabold md:text-5xl" style={{
              backgroundImage: "linear-gradient(180deg, #ff7a1a 0%, #ff4d00 45%, #c92c00 100%)",
            ...
            <p className="mt-2 text-sm md:text-base text-slate-600">Datum, locatie, disciplines en voorkeur hoofdofficial</p>
          </div>

          {err ? (
            <div className="mt-5 rounded-[18px] border px-4 py-3 text-sm font-semibold" style={{
          ...
          <div className="mt-6 grid grid-cols-1 gap-4 xl:grid-cols-2">
            <SectionCard title="Basisgegevens" subtitle="Naam, datum en locatie" icon={<CalendarDays size={22} strokeWidth={2.4} />}>
              <div className="space-y-4">
                <Field label="Naam *">
                  <input style={inputStyle()} value={naam} onChange={(e) => setNaam(e.target.value)} />
                </Field>
                ...
                <Field label="Locatie / plaats">
                  <div className="relative">
                    <MapPin size={16} strokeWidth={2.5} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#53606f" }} />
                    <input style={{ ...inputStyle(), paddingLeft: 36 }} value={locatie} onChange={(e) => setLocatie(e.target.value)} />
                  </div>
                </Field>
              </div>
            </SectionCard>

            <SectionCard title="Bondteam en disciplines" subtitle="Kies bondteam en een of meer disciplines" icon={<Shield size={22} strokeWidth={2.4} />}>
              <div className="space-y-4">
                <Field label="Bondteam *">
                  <select style={inputStyle()} value={bondteam} onChange={(e) => setBondteam((e.target.value as Bondteam) || "") }>
                ...
                <div>
                  <div className="mb-2 text-[12px] font-extrabold uppercase tracking-[0.10em] text-slate-700">Disciplines *</div>
          ...
          <div className="mt-4">
            <SectionCard title="Voorkeur official en toelichting" subtitle="Admin kan deze official overnemen of vervangen" icon={<Building2 size={22} strokeWidth={2.4} />}>
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <Field label="Voorkeur hoofdofficial">
                  <select
                    style={inputStyle()}
                    value={voorkeurHoofdofficialUserId}
                    onChange={(e) => setVoorkeurHoofdofficialUserId(e.target.value)}
                  >
