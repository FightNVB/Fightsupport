...
import { ArrowLeft, LayoutDashboard, CalendarDays, ClipboardList } from "lucide-react";
...
function statusStyle(status: string) {
  ...
  if (s === "geaccepteerd" || s === "definitief") {
    return { background: "rgba(34,197,94,0.14)", color: "#166534", border: "1px solid rgba(34,197,94,0.45)" };
  }
  if (s === "afgewezen") {
    return { background: "rgba(220,38,38,0.14)", color: "#991b1b", border: "1px solid rgba(220,38,38,0.45)" };
  }
  if (s === "verzonden_naar_official") {
    return { background: "rgba(255,77,0,0.14)", color: "#b63b00", border: "1px solid rgba(255,77,0,0.45)" };
  }
...
export default function PromotorAanvragenPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setErr("");
      try {
        const res = await authedFetch("/api/promotor/event-requests/mine", { method: "GET" });
        ...
        <div className="overflow-hidden rounded-[28px] border-[3px] border-zinc-600/60 bg-[linear-gradient(180deg,rgba(32,37,45,0.98)_0%,rgba(20,24,30,0.98)_100%)]">
          <div className="px-6 py-5" style={{ background: "linear-gradient(180deg, #3b4149 0%, #242a31 48%, #171b20 100%)", borderBottom: "3px solid rgba(255,77,0,0.5)" }}>
            <div className="grid grid-cols-1 items-center gap-4 xl:grid-cols-[1fr_auto_1fr]">
              <div>
                <div className="text-[28px] font-extrabold uppercase" style={{ color: ORANGE }}>Mijn aanvragen</div>
                <div className="mt-1 text-sm text-white/75">Overzicht van alle ingediende evenementverzoeken</div>
                <div className="mt-3 flex gap-2">
                  ...
                  <button onClick={() => router.push("/dashboard/promotor")} className="rounded-[10px] px-4 py-2 text-sm font-extrabold text-black" style={{ background: "linear-gradient(180deg,#ffffff 0%, #ececec 18%, #cfcfcf 40%, #f7f7f7 58%, #a9a9a9 100%)" }}>
          ...
          <div className="px-6 py-6">
            <div className="rounded-[24px] bg-[linear-gradient(180deg,rgba(245,247,250,0.98)_0%,rgba(229,233,238,0.98)_100%)] p-5 text-black">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <div className="text-lg font-extrabold">Mijn evenementverzoeken</div>
                  <div className="text-sm text-slate-600">Status, official en reactie</div>
                </div>
              </div>
                ...
                <div className="overflow-x-auto rounded-[18px] border-2 border-zinc-800 bg-white">
                  <table className="min-w-full border-collapse text-sm">
                    ...
                    <tbody>
                      {rows.length === 0 ? (
                        <tr>
                          <td colSpan={7} className="px-4 py-8 text-center text-slate-500">Nog geen aanvragen gevonden.</td>
                        </tr>
                      ) : (
                        rows.map((r, i) => {
                          const zebra = i % 2 === 0;
                          return (
                            <tr key={r.id} style={{ backgroundColor: zebra ? "#ffffff" : "#0d0d0d", color: zebra ? "#000" : "#fff" }}>
                              ...
                              <td className="px-4 py-3">
                                ...
                                <div className="text-xs" style={{ color: zebra ? "#64748b" : "rgba(255,255,255,0.68)" }}>{r.locatie ?? "-"}</div>
                              </td>
                              <td className="px-4 py-3">{r.bondteam ?? "-"}</td>
                              <td className="px-4 py-3">{Array.isArray(r.disciplines) ? r.disciplines.join(", ") : "-"}</td>
                              <td className="px-4 py-3">
                                <span className="inline-flex rounded-full px-3 py-1 text-xs font-semibold" style={statusStyle(r.status)}>{r.status}</span>
                              </td>
                              <td className="px-4 py-3">{r.toegewezen_hoofdofficial_name ?? r.voorkeur_hoofdofficial_name ?? "-"}</td>
                              <td className="px-4 py-3">{r.reactie_official ?? "-"}</td>
                            </tr>
                          );
                        })
                      )}
