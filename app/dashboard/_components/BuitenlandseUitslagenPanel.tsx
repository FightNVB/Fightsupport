...
function inferFighterFromNearbyDom(header: HTMLElement, hoek: Hoek) {
  ...
  for (let i = 0; i < 6 && node; i += 1) {
    ...
    if (/VA(?:-nummer)?\s*[:#]?\s*\d+/i.test(text)) break;
    node = node.parentElement;
  }

  const vaMatches = [...text.matchAll(/VA(?:-nummer)?\s*[:#]?\s*(\d+)/gi)].map((m) => m[1]);
  const va = hoek === "rood" ? vaMatches[0] : vaMatches[1] ?? vaMatches[0] ?? "";
  return { va_nummer: va || "" };
}
...
export default function BuitenlandseUitslagenPanel() {
  ...
  const pending = useMemo(
    () => rows.filter((r) => r.status !== "VERWERKT_IN_FIGHTPASSPORT" && r.status !== "AFGEWEZEN").length,
    [rows],
  );
  ...
  useEffect(() => {
    ...
    const mountButtons = () => {
      for (const { el, hoek } of findResultHeaders()) {
        ...
        button.type = "button";
        button.dataset.foreignResultsButton = "1";
        button.title = isAdmin ? "Buitenlandse uitslagen bekijken" : "Buitenlandse partij toevoegen";
        button.textContent = isAdmin ? (pending ? `Buitenlands ${pending}` : "Buitenlands") : "+";
        button.className = isAdmin
          ? "ml-2 inline-flex items-center rounded border border-[#ff4d00] bg-[#171717] px-2 py-1 text-[11px] font-black text-white hover:bg-[#242424]"
          : "ml-2 inline-flex h-7 w-7 items-center justify-center rounded border border-[#ff4d00] bg-[#171717] text-lg font-black leading-none text-white hover:bg-[#ff4d00]";
        ...
        cleanup.push(() => {
          ...
          button.remove();
          delete el.dataset.foreignResultsTarget;
        });
      }
    };
    ...
    return () => {
      ...
      document.querySelectorAll<HTMLElement>("[data-foreign-results-target='1']").forEach((el) => {
        delete el.dataset.foreignResultsTarget;
      });
      document.querySelectorAll<HTMLElement>("[data-foreign-results-button='1']").forEach((el) => el.remove());
    };
  }, [matchmakingId, partijNr, isAdmin, pending]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    setMsg("");
    try {
      const r = await authedFetch("/api/matchmaker/buitenlandse-uitslagen", {
        method: "POST",
          ...
          <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-lg border border-zinc-500 bg-[#171717] p-5 text-white shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <div className="text-xs font-black uppercase tracking-widest text-[#ff4d00]">
                  Partij {partijNr} · {activeHoek === "rood" ? "Rood" : "Blauw"}
                </div>
                <h2 className="text-xl font-black">
                  {isAdmin ? "Buitenlandse partijen controleren" : "Buitenlandse partij toevoegen"}
                </h2>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="text-2xl text-zinc-300">
                ×
              </button>
            </div>
            ...
            {!isAdmin && (
              <form onSubmit={submit} className="grid grid-cols-1 gap-3 md:grid-cols-3">
                ...
                <select value={form.uitslag} onChange={(e) => setForm({ ...form, uitslag: e.target.value })} className="rounded border border-zinc-600 bg-zinc-900 p-2">
                  <option>W</option><option>V</option><option>G</option><option>NC</option>
                </select>
            ...
            <div className="mt-5 space-y-2">
              {rows.length === 0 && <div className="text-sm text-zinc-400">Geen buitenlandse partijen gemeld voor deze partij.</div>}
              {rows.map((r) => (
                <div key={r.id} className="rounded border border-zinc-700 bg-zinc-900 p-3">
                  <div className="flex flex-wrap justify-between gap-2">
                    <b>{r.vechter_naam || "Vechter"} · VA {r.va_nummer}</b>
                    <span className="text-xs font-black text-[#ff4d00]">{r.status}</span>
                  </div>
                  <div className="mt-1 text-sm text-zinc-300">
                    {r.datum || "datum onbekend"} · {r.discipline || "discipline onbekend"} {r.klasse || ""} · {r.uitslag} vs {r.tegenstander || "onbekend"} · {r.evenement || "evenement onbekend"} · {r.land || "land onbekend"}
                  </div>
                  {r.bewijs_opmerking && <div className="mt-1 text-xs text-zinc-400">{r.bewijs_opmerking}</div>}
                  {isAdmin && r.status !== "VERWERKT_IN_FIGHTPASSPORT" && r.status !== "AFGEWEZEN" && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button type="button" onClick={() => status(r.id, "GOEDGEKEURD")} className="rounded border border-green-700 px-3 py-1 text-xs font-bold">Goedkeuren</button>
                      <button type="button" onClick={() => status(r.id, "VERWERKT_IN_FIGHTPASSPORT")} className="rounded bg-[#ff4d00] px-3 py-1 text-xs font-bold">Verwerkt in Fightpassport</button>
                      <button type="button" onClick={() => status(r.id, "AFGEWEZEN")} className="rounded border border-red-700 px-3 py-1 text-xs font-bold">Afwijzen</button>
                    </div>
                  )}
                </div>
              ))}
