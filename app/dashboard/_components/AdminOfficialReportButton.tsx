...
export default function AdminOfficialReportButton() {
  ...
  useEffect(() => {
    const reportMatch = pathname.match(/^\/dashboard\/admin\/controle\/([^/]+)\/rapport\/?$/);
    if (reportMatch) {
      ...
      const fixBackLink = () => {
        ...
        const backLink = links.find((link) => {
          ...
          return label === "← terug" && href.includes(`/dashboard/matchmaker/matchmaking/${matchmakingId}`);
        });
    ...
    const mount = () => {
      ...
      // Admin heeft Sportdata altijd beschikbaar. Herstel de knop als oudere code hem verborgen heeft.
      ...
      button.addEventListener("click", (event) => {
        ...
        router.push(`/dashboard/admin/controle/${encodeURIComponent(matchmakingId)}/eindrapport`);
      });
    ...
    observer.observe(document.body, { childList: true, subtree: true });
    return () => { observer.disconnect(); document.getElementById(BUTTON_ID)?.remove(); };
  }, [pathname, router]);
