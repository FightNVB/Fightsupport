...
function leeftijdOpEventGetal(ctx: AnyRow | null | undefined, side: "rood" | "blauw"): number | null {
  ...
  const eventDate = parseISODateOnly(ctx?.evenement_datum ?? ctx?.datum);
...
function ageYearsAtEvent(ctx: AnyRow, side: "rood" | "blauw"): string {
  const event = parseISODateOnly(ctx?.evenement_datum ?? ctx?.datum);
...
function wedstrijddetailsFromCtx(ctx: AnyRow | null | undefined): {
  ...
  if (isMma) {
    if (jongste != null && jongste < 18) {
      if (jongste >= 16) {
        return { rondeTijd: formatMinutesToClock(2.5), format: "3 x 2:30", rustTijd: "1:00" };
      }
      if (jongste >= 14) {
        return { rondeTijd: formatMinutesToClock(2), format: "3 x 2:00", rustTijd: "1:00" };
      }
      if (jongste >= 12) {
        return { rondeTijd: formatMinutesToClock(1.5), format: "3 x 1:30", rustTijd: "1:00" };
      }
    }
    ...
    if (
      ...
      return { rondeTijd: formatMinutesToClock(5), format: "3 x 5:00", rustTijd: null };
    }

    return { rondeTijd: formatMinutesToClock(3), format: "3 x 3:00", rustTijd: null };
  }
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 items-center gap-4">
          ...
          <div className="order-3 md:text-right">
            <div className="text-[11px] tracking-widest text-white/60 font-semibold">FLOW</div>
        ...
        <div className="flex items-center justify-between gap-3 p-3 border-t border-zinc-300">
          ...
          {hasMore ? (
            <button
              ...
              className="px-3 py-2 rounded text-black text-xs font-extrabold hover:opacity-90"
...
export default function PartijDetailPage() {
  ...
  const isDispensatieAdmin = useMemo(() => {
    const lower = roleNames.map((r) => r.toLowerCase());
    return lower.includes("dispensatie_admin") || lower.includes("superadmin");
  }, [roleNames]);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const partijFilterRunId = latestRun?.id ?? null;
        ...
        if (partijFilterRunId) {
          pnQuery = pnQuery.eq("controle_run_id", partijFilterRunId);
        }

        const { data: pnRows, error: pnErr } = await pnQuery;
        ...
        if (partijFilterRunId) {
          ctxQuery = ctxQuery.eq("controle_run_id", partijFilterRunId);
        }
        ...
        if (partijFilterRunId) {
          regelsQuery = regelsQuery.eq("controle_run_id", partijFilterRunId);
        }

        const { data: resRows, error: resErr } = await regelsQuery;
  ...
  const header = useMemo(() => {
    ...
    const roodGym = ctx?.rood_gym_mm ?? ctx?.rood_gym ?? "-";
    const blauwGym = ctx?.blauw_gym_mm ?? ctx?.blauw_gym ?? "-";

    const discipline = ctx?.discipline ?? "-";
    const klasseMM = ctx?.klasse_mm ?? ctx?.klasse ?? "-";

    const roodDob =
      ctx?.rood_geboortedatum_fp ?? ctx?.rood_geboortedatum_mm ?? ctx?.rood_geboortedatum ?? null;
    const blauwDob =
      ctx?.blauw_geboortedatum_fp ?? ctx?.blauw_geboortedatum_mm ?? ctx?.blauw_geboortedatum ?? null;
  ...
  }, [ctx, evenementNaam, evenementDatum]);
  ...
  const verschillen = useMemo(() => {
    ...
    const rBirth = parseISODateOnly(
      ctx?.rood_geboortedatum_fp ?? ctx?.rood_geboortedatum_mm ?? ctx?.rood_geboortedatum
    );
    const bBirth = parseISODateOnly(
      ctx?.blauw_geboortedatum_fp ?? ctx?.blauw_geboortedatum_mm ?? ctx?.blauw_geboortedatum
    );

    const leeftijdDiff = eventDate && rBirth && bBirth ? diffMonthsDaysAbs(rBirth, bBirth) : null;

    const roodPartijen = toInt(ctx?.rood_totaal_wedstrijden_scrape ?? ctx?.rood_totaal_wedstrijden);
    const blauwPartijen = toInt(ctx?.blauw_totaal_wedstrijden_scrape ?? ctx?.blauw_totaal_wedstrijden);
    ...
    const roodEffectief =
      roodPartijen != null ? roodPartijen - (roodDemo ?? 0) + Math.floor((roodDemo ?? 0) / 3) : null;
    const blauwEffectief =
      blauwPartijen != null ? blauwPartijen - (blauwDemo ?? 0) + Math.floor((blauwDemo ?? 0) / 3) : null;

    const partijenVerschil =
      roodEffectief != null && blauwEffectief != null ? Math.abs(roodEffectief - blauwEffectief) : null;

    return {
      maandenVerschil: leeftijdDiff ? leeftijdDiff.months : null,
      dagenVerschil: leeftijdDiff ? leeftijdDiff.days : null,
      leeftijdVerschilTekst: leeftijdDiff ? fmtMonthsDays(leeftijdDiff.months, leeftijdDiff.days) : null,
  ...
  const gewichtInfo = useMemo(() => {
    ...
    const rKg = toNumKg(ctx?.rood_gewicht_mm ?? ctx?.rood_gewicht);
    const bKg = toNumKg(ctx?.blauw_gewicht_mm ?? ctx?.blauw_gewicht);
    const explicitMaxKg = toNumKg(ctx?.max_gewicht ?? ctx?.max_gewicht_mm);
  ...
  async function sendToDispensatie() {
    try {
      ...
      const bout_id = asUuid((ctx as any)?.bout_id ?? (ctx as any)?.id);
      if (!bout_id) throw new Error("bout_id ontbreekt/ongeldig in context.");
      ...
      const best = [...(regels ?? [])].sort((a, b) => prio(b) - prio(a))[0];
      if (!best?.rule_code) {
    ...
    <Shell>
      <div className="space-y-4">
        <BruteHeaderA
          ...
          onPrev={() => nav.prev && router.push(`/dashboard/matchmaker/matchmaking/${matchmakingId}/${nav.prev}`)}
          onNext={() => nav.next && router.push(`/dashboard/matchmaker/matchmaking/${matchmakingId}/${nav.next}`)}
        />
        ...
        >
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px_1fr] gap-4 items-start">
            <div className="order-1">
              <FighterMetalCard
                ...
                gym={header.roodGym}
                va={String(ctx?.rood_va_mm ?? ctx?.rood_va ?? ctx?.va_rood ?? "-")}
                lic={header.roodLic}
                sv={header.roodSv}
                dob={header.roodDob}
            ...
            <div className="order-3">
              <FighterMetalCard
                ...
                gym={header.blauwGym}
                va={String(ctx?.blauw_va_mm ?? ctx?.blauw_va ?? ctx?.va_blauw ?? "-")}
                lic={header.blauwLic}
                sv={header.blauwSv}
                dob={header.blauwDob}
              ...
              <div className="rounded-xl p-3" style={darkInsetStyle()}>
                ...
                <div className="mt-2 text-white/90">
                  ...
                  <span className="text-white font-extrabold">
                    {verschillen?.leeftijdVerschilTekst != null ? verschillen.leeftijdVerschilTekst : "-"}
                  </span>
                </div>
              ...
              <div className="rounded-xl p-3" style={darkInsetStyle()}>
                <div className="flex items-center justify-between gap-2">
                  ...
                  {(() => {
                    ...
                    const okUnder =
                      klasseMaxKg == null || ((rKg == null || rKg <= klasseMaxKg) && (bKg == null || bKg <= klasseMaxKg));
                ...
                <div className="mt-2 space-y-1 text-white/85">
                  ...
                  <div>
                    Max gewicht:{" "}
                    <span className="text-white font-extrabold">
                      {gewichtInfo?.klasseMaxKg != null ? `${gewichtInfo.klasseMaxKg.toFixed(1)} kg` : "-"}
                    </span>
                  ...
                  <div>
                    Verschil:{" "}
                    <span className="text-white font-extrabold">{gewichtInfo?.diffKg != null ? `${gewichtInfo.diffKg.toFixed(1)} kg` : "-"}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="rounded-2xl overflow-hidden mt-4" style={{ ...plateBodyStyle(), padding: 0 }}>
            <div className="p-3">
            ...
            <div className="p-4 pt-2">
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <button
                  ...
                  disabled={rescraping}
                  className="inline-flex items-center px-4 py-2 rounded text-black font-semibold hover:opacity-90 disabled:opacity-50"
                  style={{ background: NVB_ORANGE }}
                >
