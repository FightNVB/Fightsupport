...
/**
 * ✅ Route-guard Matchmaker portaal.
 * Toegestaan: matchmaker, admin, superadmin
 */
export default function MatchmakerLayout({ children }: { children: React.ReactNode }) {
  const router = useRouter();
  const { user, roles, loading } = useAuth();

  const allowed = (roles ?? []).includes("matchmaker") || (roles ?? []).includes("admin") || (roles ?? []).includes("superadmin");
