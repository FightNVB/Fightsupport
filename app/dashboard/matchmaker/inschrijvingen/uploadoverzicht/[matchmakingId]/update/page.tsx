...
function DispensatieLinks({ meldingen }: { meldingen: ResultRow[] }) {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="inline-flex items-center rounded-md bg-[#111] px-3 py-1.5 text-xs font-bold text-[var(--brand-silver)] ring-1 ring-white/15 hover:bg-[#161616] hover:text-white"
...
export default function RapportPage() {
  ...
  return (
    <div className="min-h-screen bg-[#070707] text-white fs-report">
      ...
      <style jsx global>{`
        @media print {
          ...
          /* ✅ Fightsupport PDF: meer zilver + randen per partij (rij) */
          .fs-report .fs-silver-frame {
            box-shadow: 0 0 0 2px rgba(192,192,192,0.55) !important;
          }
          .fs-report table {
            border-collapse: separate !important;
            border-spacing: 0 6px !important; /* ruimte tussen partijen */
          }
      ...
      <div className="print-max mx-auto max-w-6xl px-4 py-6">
        ...
        <div className="fs-silver-frame rounded-[28px] p-[2px] bg-gradient-to-b from-white/45 via-white/15 to-white/35 shadow-[0_0_0_1px_rgba(255,255,255,0.10),0_30px_120px_rgba(0,0,0,0.85)]">
          <div className="rounded-[26px] p-[2px] bg-gradient-to-b from-[#0b0b0b] via-[#070707] to-[#0b0b0b] ring-1 ring-black/70">
            <div className="rounded-[24px] bg-[#0b0b0b] ring-1 ring-[#ff4d00]/25">
              {/* header */}
              <div className="px-5 py-5 border-b border-white/15">
                <div className="grid gap-4 lg:grid-cols-3 lg:items-center">
                  {/* links: info */}
                  <div>
                    <div className="text-xs font-extrabold tracking-wider text-white/60">FIGHTSUPPORT RAPPORT</div>
                    <div className="mt-2">
                      <div className="text-[#ff4d00] font-extrabold text-sm">Evenement</div>
                      <div className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">{eventMeta?.naam ?? "-"}</div>
                      <div className="mt-1 text-[#ff4d00] font-extrabold text-sm">Datum</div>
                      <div className="text-xl sm:text-2xl font-extrabold tracking-tight text-white/90">{fmtNlDateOnly(eventMeta?.datum)}</div>
                    </div>
                  ...
                  <div className="flex justify-center">
                    <div className="rounded-xl p-[5px] bg-gradient-to-b from-[#e0e0e0] via-[#8c8c8c] to-[#cfcfcf]">
                      <div className="rounded-lg border-[3px] border-[#bdbdbd] bg-[#0b0b0b] p-5">
                        <FsLogo className="h-32 sm:h-36 w-auto" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="no-print flex items-center justify-start lg:justify-end gap-2">
                    <Link
                      ...
                      className="rounded-md bg-[#0f0f0f] px-3 py-2 text-sm font-semibold text-[#c0c0c0] ring-1 ring-white/20 hover:bg-[#151515] hover:text-white"
                    >
                      Terug
                    </Link>
                    <button
              ...
              <div className="px-5 py-6 space-y-6">
                {loading && <div className="rounded-xl border border-white/15 bg-[#0f0f0f] p-4 text-sm text-white/80">Rapport laden…</div>}
                ...
                {!loading && !error && (
                  <>
                    ...
                    <div className="rounded-2xl border border-white/12 bg-[#0f0f0f] overflow-hidden">
            ...
            <div className="rounded-2xl border border-white/12 bg-[#0f0f0f] overflow-hidden">
              ...
              <div className="p-3 space-y-4">
                ...
                <div className="rounded-xl border border-white/15 bg-[#0b0b0b] p-3">
                  <div className="flex items-center justify-between gap-3">
                    <div className="font-semibold">Fightpaspoort nummer gewijzigd</div>
                    <span className="text-xs font-extrabold text-white/70">{lijstFightpaspoortGewijzigd.length}</span>
                  </div>

                  {lijstFightpaspoortGewijzigd.length === 0 ? (
                    <div className="mt-2 text-sm text-white/60">Geen.</div>
                  ) : (
                    <div className="mt-3">
                      <NvbTable columns={["Partij", "Hoek", "Naam", "Sportschool", "Melding"]}>
                        {lijstFightpaspoortGewijzigd.map((x, i) => (
                          <tr key={`fpchg-${x.partij_nr}-${x.hoek}-${i}`}>
                            ...
                            <td className="px-4 py-2">
                              <div>{x.boodschap}</div>
                              {x.created_at ? <div className="mt-1 text-xs text-white/50">({fmtDateTime(x.created_at)})</div> : null}
                            </td>
                          </tr>
                        ))}
                      </NvbTable>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Licentie */}
              <div className="rounded-2xl border border-white/12 bg-[#0f0f0f] overflow-hidden">
                <div className="bg-[#101010] text-white px-4 py-3 font-extrabold border-b border-white/15">Licentie (nee/onbekend)</div>
                <div className="p-3">
                  {lijstLicentie.length === 0 ? (
                    <div className="text-sm text-white/60">Geen.</div>
                  ) : (
                    <ul className="space-y-2 text-sm">
                      {lijstLicentie.map((x, idx) => (
                        <li key={`lic-${idx}`} className="rounded-xl border border-white/15 bg-[#0b0b0b] p-2">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              {x.ookGeenVa ? (
                                <span className="rounded bg-[#ff4d00]/15 px-2 py-0.5 text-xs font-extrabold text-[#ff4d00] ring-1 ring-[#ff4d00]/25">
                                  ook geen VA
                                </span>
                              ) : null}
                              <span className="text-xs font-extrabold text-red-300">licentie: {x.licentie}</span>
                            </div>
                          </div>
              ...
              <div className="rounded-2xl border border-white/12 bg-[#0f0f0f] overflow-hidden">
                <div className="bg-[#101010] text-white px-4 py-3 font-extrabold border-b border-white/15">Keurmerk (sportscholen)</div>
                <div className="p-3">
                  <div className="text-xs text-white/45 mb-2">Ongeldig of onbekend (uniek)</div>
                  {lijstKeurmerkSportscholen.length === 0 ? (
                    <div className="text-sm text-white/60">Geen.</div>
                  ) : (
                    <ul className="space-y-1 text-sm">
                      {lijstKeurmerkSportscholen.map((gym, idx) => (
                        <li key={`keur-${idx}`} className="rounded-lg border border-white/15 bg-[#0b0b0b] px-2 py-1">
                          {gym}
                        </li>
                      ))}
                    </ul>
              ...
              {/* Startverbod */}
              <div className="rounded-2xl border border-white/12 bg-[#0f0f0f] overflow-hidden">
                <div className="bg-[#101010] text-white px-4 py-3 font-extrabold border-b border-white/15">Startverbod (true)</div>
                <div className="p-3">
                  {lijstStartverbod.length === 0 ? (
                    ...
                    <ul className="space-y-2 text-sm">
                      {lijstStartverbod.map((x, idx) => (
                        <li key={`sv-${idx}`} className="rounded-xl border border-red-500/25 bg-red-500/10 p-2">
                          <div className="font-semibold">
                            {x.partij} • {x.hoek}
                          </div>
                          <div className="mt-1">{x.naam}</div>
                          <div className="text-xs text-white/60">{x.gym}</div>
                        </li>
                      ))}
                    </ul>
                  )}
            ...
            <div className="space-y-6">
              {partijMetMeldingen.map((it, idx) => (
                <div key={`partij-${safe(it.partij_nr)}-${idx}`} className="relative mb-12 rounded-2xl p-[4px] bg-gradient-to-br from-[#d9d9d9] via-[#8f8f8f] to-[#cfcfcf]">
                  <div className="rounded-[14px] border border-white/12 bg-[#0f0f0f] overflow-hidden">
                  ...
                  <div className="px-4 py-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="rounded-xl border border-white/15 bg-[#0b0b0b] p-3">
                        <div className="text-xs font-semibold text-white/60">Rood</div>
                      ...
                      <div className="rounded-xl border border-white/15 bg-[#0b0b0b] p-3">
                        <div className="text-xs font-semibold text-white/60">Blauw</div>
