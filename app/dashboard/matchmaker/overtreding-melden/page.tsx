...
const BETROKKENEN = ["matchmaker", "vechter", "matchmaker", "sportschool", "anders"];
...
export default function MatchmakerOvertredingMeldenPage() {
  ...
  useEffect(() => {
    ...
    async function loadMelder() {
      ...
      try {
        ...
        // De API vult minimaal bron=matchmaker; opslaan blijft mogelijk.
      }
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/matchmaker/overtreding-melden", {
      method: "POST",
    ...
    setSuccess("Melding is doorgestuurd naar Admin > Algemeen > Overtredingen > Ontvangen van matchmaker.");
    setForm({
      ...initialForm,
      datum_overtreding: todayIso(),
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 max-w-3xl text-xs font-semibold text-zinc-300">
                  Meld een overtreding of incident. De melding komt direct binnen bij admin in de tab Ontvangen van matchmaker.
                </p>
              </div>
          ...
          <div className="grid gap-2 p-3 md:grid-cols-3">
            ...
            <div className="border border-zinc-600/70 bg-black/35 p-2">
              ...
              <div className="mt-1 text-sm font-black text-zinc-200">
                {form.melder_naam || "Matchmaker"}{form.melder_bondteam ? ` · ${form.melder_bondteam}` : ""}
              </div>
            </div>
          </div>
        </header>
        ...
        <form onSubmit={submit} className="border border-orange-500/40 bg-[#211f1d] p-4 shadow-xl">
          ...
          <div className="grid gap-2 md:grid-cols-4">
            ...
            <label className="block text-[10px] font-black uppercase text-zinc-400 md:col-span-2">
              Naam betrokkene
              <input required className={fieldClass()} value={form.naam} onChange={(e) => setValue("naam", e.target.value)} placeholder="Naam matchmaker, vechter, matchmaker of sportschool" />
            </label>
