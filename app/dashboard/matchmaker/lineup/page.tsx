...
function StatusBadge({ status }: { status: string | null }) {
  ...
  if (lower === "definitief" || lower === "klaar" || lower === "goedgekeurd") {
    return (
      <span
        className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold"
        style={{
  ...
  if (lower === "concept" || lower === "in_progress" || lower === "bezig") {
    return (
      <span
        className="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold"
        style={{
...
export default function MatchmakerLineupOverzichtPage() {
  ...
  useEffect(() => {
    ...
    const allowed = roles?.some((r) =>
      ["matchmaker", "admin", "superadmin"].includes(r)
    );
    if (!allowed) {
  ...
  async function load() {
    ...
    try {
      const [{ data: uploads, error: uploadsError }, { data: bouts, error: boutsError }] =
        await Promise.all([
          supabase
            ...
            .select("matchmaking_id, evenement_naam, evenement_datum, bondteam, status")
      ...
      const rows: MatchmakingRow[] = (uploads ?? []).map((upload) => ({
                  ...
                  <div className="mt-2 text-center">
                    <h1
                      ...
                      style={{
                        ...
                        backgroundClip: "text",
                        color: "transparent",
                        textShadow: "0 2px 0 rgba(255,255,255,0.35), 0 8px 22px rgba(0,0,0,0.35)",
                      }}
                    >
                      Lineup Overzicht
                    </h1>
                    ...
                    <p className="mt-2 text-sm md:text-base text-zinc-700">
                      Alle matchmakings — selecteer een evenement om de lineup te bewerken
                    </p>
                  </div>
                            ...
                            {rows.map((row, i) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="py-3 px-4">
                                    <Link
                                      href={`/dashboard/matchmaker/lineup/${encodeURIComponent(row.matchmaking_id)}`}
