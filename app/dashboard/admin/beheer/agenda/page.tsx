...
import { ArrowLeft, CalendarDays, RefreshCw, Search, MapPin, UsersRound, Trash2 } from "lucide-react";
...
function eventName(event: EventRow) {
  return pick(event.naam, event.event_naam, event.titel, event.name) || "Onbekend evenement";
}
...
export default function AdminAgendaPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!res.ok || !json?.ok) throw new Error(json?.error || "Agenda laden mislukt");
  ...
  async function deleteEvent(event: EventRow) {
    ...
    const akkoord = window.confirm(
      `Weet je zeker dat je evenement "${naam}" wilt verwijderen?`,
    );
    ...
    <main className="min-h-screen bg-[#2b2b2b] p-4 text-white sm:p-6">
      ...
      <section className="mx-auto max-w-7xl border border-zinc-500 bg-[#121212] shadow-2xl">
        <header className="border-b border-zinc-600 bg-gradient-to-r from-[#1d1d1d] via-[#303030] to-[#151515] p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h1 className="text-2xl font-black uppercase">Agenda evenementen</h1>
              <p className="text-sm text-zinc-300">Overzicht van alle evenementen uit de events-tabel.</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link className="agenda-silver-btn border border-zinc-300 bg-gradient-to-b from-white via-zinc-200 to-zinc-500 px-4 py-2 text-sm font-black uppercase !text-black" href="/dashboard/admin/beheer">
                <ArrowLeft className="mr-2 inline" size={16} /> Terug naar beheer
              </Link>
              <button onClick={load} className="border border-[#ff4d00] bg-[#ff4d00] px-4 py-2 text-sm font-black uppercase !text-black">
                <RefreshCw className={`mr-2 inline ${loading ? "animate-spin" : ""}`} size={16} /> Vernieuwen
              </button>
            </div>
          </div>
        </header>
        ...
        <div className="grid gap-3 border-b border-zinc-700 p-4 md:grid-cols-3">
          <Stat label="Evenementen" value={stats.total} icon={<CalendarDays size={18} />} />
          <Stat label="Aankomend" value={stats.upcoming} icon={<UsersRound size={18} />} />
          <Stat label="Historie" value={stats.past} icon={<MapPin size={18} />} />
        </div>

        <div className="flex flex-wrap gap-2 p-4">
          {["alles", "open", "concept", "actief", "afgerond", "geannuleerd"].map((t) => (
            <button key={t} onClick={() => setStatus(t)} className={`border px-3 py-2 text-xs font-black uppercase ${status === t ? "border-[#ff4d00] bg-[#ff4d00] !text-black" : "border-zinc-500 bg-[#242424] text-white"}`}>
              {t}
            </button>
          ))}
          <form onSubmit={(e) => { e.preventDefault(); load(); }} className="ml-auto flex min-w-[280px] flex-1 justify-end gap-2">
            <div className="flex min-w-[240px] flex-1 items-center gap-2 border border-zinc-600 bg-[#111] px-3 py-2">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Zoek event, locatie, promotor..." className="w-full bg-transparent text-sm text-white outline-none" />
            </div>
            <button className="agenda-silver-btn border border-zinc-300 bg-gradient-to-b from-white via-zinc-200 to-zinc-500 px-4 py-2 text-sm font-black !text-black">Zoek</button>
          </form>
        </div>
        ...
        <div className="overflow-x-auto p-4 pt-0">
          <table className="w-full border-collapse text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="border border-zinc-800 p-4 text-zinc-300">Laden...</td></tr>
              ) : items.length ? (
                items.map((event, idx) => {
                  ...
                  const borderClass = isLight ? "border-zinc-300" : "border-zinc-800";

                  return (
                    <tr key={event.id || idx} className={rowClass}>
                      <td className={`border p-2 ${borderClass}`}><b className="text-[#ff4d00]">{eventName(event)}</b><br /><span className={`text-xs ${mutedClass}`}>{event.id || "-"}</span></td>
                      <td className={`border p-2 font-bold ${borderClass}`}>{formatDate(eventDate(event))}</td>
                      <td className={`border p-2 ${borderClass}`}>{eventLocation(event)}</td>
                      <td className={`border p-2 ${borderClass}`}>{pick(event.promotor, event.promotor_naam) || "-"}</td>
                      <td className={`border p-2 ${borderClass}`}>{pick(event.matchmaker, event.matchmaker_naam) || "-"}</td>
                      <td className={`border p-2 ${borderClass}`}>{pick(event.hoofdofficial, event.hoofdofficial_naam) || "-"}</td>
                      <td className={`border p-2 ${borderClass}`}><span className={`border px-2 py-1 text-xs font-black uppercase ${isLight ? "border-zinc-500 text-zinc-900" : "border-zinc-500 text-zinc-200"}`}>{eventStatus(event)}</span></td>
                ...
                <tr><td colSpan={8} className="border border-zinc-800 p-6 text-center text-zinc-400">Geen evenementen gevonden.</td></tr>
              )}
            </tbody>
          </table>
        </div>
