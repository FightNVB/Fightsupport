...
export default function TalentstatusRapportagePage() {
  ...
  return <main className="min-h-screen bg-[#2b2b2b] p-6 text-white"><section className="mx-auto max-w-7xl border border-zinc-500 bg-[#121212] shadow-2xl">
    ...
    <div className="grid gap-4 border-b border-zinc-700 p-4 md:grid-cols-3"><Stat value={partijen.length} label="Partijen in rapport" /><Stat value={vechters.length} label="Talentstatus vechters" /><Stat value="1" label="PDF knop" /></div>
    ...
    <section className="p-4">
      <div className="mx-auto max-w-5xl overflow-hidden border border-zinc-400 bg-[#eeeeee] !text-black shadow-2xl">
        ...
        <div className="grid gap-3 bg-[#d9d9d9] p-4 md:grid-cols-3"><PreviewStat value={partijen.length} label="Partijen" /><PreviewStat value={vechters.length} label="Vechters" /><PreviewStat value={new Date().toLocaleDateString("nl-NL")} label="Datum" /></div>
        <div className="p-5">
          <h3 className="border-b-2 border-[#ff4d00] pb-2 text-lg font-black uppercase">Partijen</h3>
          <div className="mt-3 overflow-x-auto"><table className="w-full border-collapse text-sm"><thead className="bg-[#222] text-left text-xs uppercase text-white"><tr><th className="p-2">Datum/event</th><th className="p-2">Vechter 1</th><th className="p-2">Vechter 2</th><th className="p-2">Winnaar</th><th className="p-2">Controle</th></tr></thead><tbody>{loading ? <tr><td colSpan={5} className="p-3">Laden...</td></tr> : filteredPartijen.map(p => <tr key={p.id} className="border-b border-zinc-300 bg-white"><td className="p-2"><b>{p.event_naam || "-"}</b><br/><span className="text-xs text-zinc-600">{p.event_datum || "geen datum"}</span></td><td className="p-2"><b>{p.vechter_naam}</b><br/><span className="text-xs text-zinc-600">{p.vechter_land} {p.vechter_va ? `VA ${p.vechter_va}` : ""}</span></td><td className="p-2"><b>{p.tegenstander_naam}</b><br/><span className="text-xs text-zinc-600">{p.tegenstander_land} {p.tegenstander_va ? `VA ${p.tegenstander_va}` : ""}</span></td><td className="p-2">{p.winnaar || "-"}<br/><span className="text-xs text-zinc-600">{p.uitslag || ""}</span></td><td className="p-2"><b className={p.vechter_talentstatus_ontbreekt || p.tegenstander_talentstatus_ontbreekt ? "text-red-700" : "text-green-700"}>{p.vechter_talentstatus_ontbreekt || p.tegenstander_talentstatus_ontbreekt ? "Status ontbreekt" : "OK"}</b></td></tr>)}</tbody></table></div>
