...
function leeftijd(geboortedatum?: string | null) {
...
export default function TalentstatusVechtersOverzichtPage() {
  ...
  return <main className="min-h-screen bg-[#2b2b2b] p-6 text-white"><style>{`.talent-silver-btn, .talent-silver-btn *{color:#000!important;}`}</style>
    <section className="mx-auto max-w-7xl border border-zinc-500 bg-[#121212] shadow-2xl">
      <header className="border-b border-zinc-600 bg-gradient-to-r from-[#1d1d1d] via-[#303030] to-[#151515] p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="text-2xl font-black uppercase">Overzicht talentstatus vechters</h1>
            <p className="text-sm text-zinc-300">Zoeken, controleren en PDF-overzicht maken van alle talentstatus-vechters.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link className="talent-silver-btn border border-zinc-300 bg-gradient-to-b from-white via-zinc-200 to-zinc-500 px-4 py-2 text-sm font-black uppercase !text-black" href="/dashboard/admin/beheer/talentstatus">Terug</Link>
            ...
            <Link className="talent-silver-btn border border-zinc-300 bg-gradient-to-b from-white via-zinc-200 to-zinc-500 px-4 py-2 text-sm font-black uppercase !text-black" href="/dashboard/admin/beheer/talentstatus/rapportage">Rapportage / PDF</Link>
          </div>
        </div>
      </header>
      ...
      <div className="grid gap-3 border-b border-zinc-700 p-4 md:grid-cols-3">
        ...
        <div className="border border-zinc-600 bg-[#1c1c1c] p-3"><b className="text-xl text-[#ff4d00]">PDF</b><p className="text-xs uppercase text-zinc-400">Rapportage beschikbaar</p></div>
      </div>
      ...
      <div className="overflow-x-auto p-4 pt-0">
        <table className="w-full border-collapse text-sm">
          <thead className="bg-[#252525] text-left text-xs uppercase text-zinc-300">
            <tr><th className="border border-zinc-700 p-2">Naam</th><th className="border border-zinc-700 p-2">VA</th><th className="border border-zinc-700 p-2">Leeftijd</th><th className="border border-zinc-700 p-2">Sportschool</th><th className="border border-zinc-700 p-2">Land</th><th className="border border-zinc-700 p-2">Talentstatus</th><th className="border border-zinc-700 p-2">Partijen</th><th className="border border-zinc-700 p-2">Actie</th></tr>
          </thead>
          <tbody>
            {loading ? <tr><td colSpan={8} className="p-4">Laden...</td></tr> : items.map(item => <tr key={item.id} className="bg-[#171717] hover:bg-[#202020]">
              <td className="border border-zinc-800 p-2"><b className="text-[#ff4d00]">{item.naam}</b></td>
              <td className="border border-zinc-800 p-2">{item.va_nummer ? `VA ${item.va_nummer}` : "-"}</td>
              <td className="border border-zinc-800 p-2">{leeftijd(item.geboortedatum)}</td>
              <td className="border border-zinc-800 p-2">{item.sportschool || "-"}</td>
              <td className="border border-zinc-800 p-2">{item.land}</td>
              <td className="border border-zinc-800 p-2"><span className={`border px-2 py-1 text-xs font-black uppercase ${item.moet_evalueren || item.status === "evaluatie_nodig" ? "border-[#ff4d00] text-[#ff4d00]" : "border-zinc-500 text-zinc-200"}`}>{item.moet_evalueren ? "evaluatie nodig" : item.talent_status}</span></td>
