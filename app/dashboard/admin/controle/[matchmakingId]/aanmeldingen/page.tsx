...
type BusyMode = "idle" | "upload" | "manual" | "controle" | "delete" | "save" | "load";
type ViewMode = "latest_open" | "all_open" | "scraped" | "failed" | "running" | "all";
...
function pickName(r: Aanmelding) {
  ...
  const last = pick(r, ["achternaam", "last_name", "lastname", "tussenvoegsel_achternaam"]);
  ...
  if (combined && (!direct || direct.toLowerCase() === first.toLowerCase())) return combined;
...
function uploadId(r: Aanmelding) {
  return pick(r, ["upload_batch_id", "upload_id", "aanmelding_upload_id", "source_upload_id", "batch_id"]);
}
...
function statusOf(r: Aanmelding) {
  const normalized = normalizeStatus(pick(r, [
    "status",
    "aanmelding_status",
    "controle_status",
    "scrape_status",
  ...
  const hasScrapeSignal =
    ...
    hasValue(r?.scrape_run_id) ||
    hasValue(r?.scrape_started_at) ||
    hasValue(r?.controle_run_id) ||
    hasValue(r?.checked_at) ||
    hasValue(r?.fightpaspoort_checked_at);

  // Belangrijk: oudere/lopende routes zetten soms alleen scrape_run_id/scrape_started_at
  // of blijven op status controle_bezig staan. Die horen niet meer onder rauw/open,
  // maar onder Gescrapt, zodat de pagina duidelijk laat zien dat ze al door de controle zijn geweest.
...
function isRunningAanmelding(r: Aanmelding) {
  return false;
}

function isOpenAanmelding(r: Aanmelding) {
  const status = statusOf(r);
  return status === "rauw" || status === "scrape_mislukt";
}
...
function rowToForm(r: Aanmelding): FighterForm {
  return {
    ...
    telefoon: pick(r, ["telefoon", "phone", "trainer_telefoon", "contact_telefoon"]),
  };
}
...
function busySubText(mode: BusyMode) {
  if (mode === "controle") return "De geselecteerde aanmeldingen worden gecontroleerd. Ze blijven zichtbaar en krijgen daarna status gescrapt of scrape mislukt.";
  if (mode === "upload") return "Het bestand wordt opgeslagen en toegevoegd aan deze matchmaking.";
  return "Deze actie wordt uitgevoerd.";
}

function sortAanmeldingen(list: Aanmelding[]) {
  return [...list].sort((a, b) => {
    const discipline = pick(a, ["discipline", "sport"]).localeCompare(pick(b, ["discipline", "sport"]), "nl", { sensitivity: "base" });
    if (discipline !== 0) return discipline;

    const klasse = pick(a, ["klasse", "klasse_mm"]).localeCompare(pick(b, ["klasse", "klasse_mm"]), "nl", { sensitivity: "base" });
    if (klasse !== 0) return klasse;

    const geslacht = pick(a, ["geslacht", "gender"]).localeCompare(pick(b, ["geslacht", "gender"]), "nl", { sensitivity: "base" });
...
export default function AanmeldingenPage() {
  ...
  const [viewMode, setViewMode] = useState<ViewMode>("latest_open");
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setMatchmaking(json?.matchmaking || json?.data?.matchmaking || json?.event || null);
    } catch (e: any) {
  ...
  const latestUploadId = useMemo(() => {
    const newest = uploadTabs[0];
    return s(newest?.upload_id || newest?.id || newest?.upload_batch_id);
  }, [uploadTabs]);

  const rawRows = useMemo(() => sortAanmeldingen(rows.filter(isRawAanmelding)), [rows]);
  const openRows = useMemo(() => sortAanmeldingen(rows.filter(isOpenAanmelding)), [rows]);
  const scrapedRows = useMemo(() => sortAanmeldingen(rows.filter(isScrapedAanmelding)), [rows]);
  const failedRows = useMemo(() => sortAanmeldingen(rows.filter(isFailedAanmelding)), [rows]);
  const runningRows = useMemo(() => sortAanmeldingen(rows.filter(isRunningAanmelding)), [rows]);
  const allRows = useMemo(() => sortAanmeldingen(rows), [rows]);

  const latestOpenRows = useMemo(() => {
    if (!latestUploadId) return openRows;
    return openRows.filter((r) => uploadId(r) === latestUploadId);
  }, [openRows, latestUploadId]);

  const visibleRows = useMemo(() => {
    if (viewMode === "scraped") return scrapedRows;
    if (viewMode === "failed") return failedRows;
    if (viewMode === "running") return runningRows;
    if (viewMode === "all") return allRows;
    if (viewMode === "all_open") return openRows;
    return latestOpenRows;
  }, [viewMode, scrapedRows, failedRows, runningRows, allRows, openRows, latestOpenRows]);

  const rowsForCheck = viewMode === "all_open" || viewMode === "failed" ? openRows : latestOpenRows;

  const eventName = pick(matchmaking || {}, ["naam", "event_naam", "titel", "title", "naam_event", "wedstrijdnaam"]);
  const eventDate = fmtDate(pick(matchmaking || {}, ["datum", "event_datum", "event_date", "evenement_datum", "date"]));
  ...
  async function upload() {
    ...
    try {
      ...
      setViewMode("latest_open");
      setMsg(json?.message || "Upload gelukt. Nieuwe aanmeldingen zijn toegevoegd aan deze matchmaking.");
      await load(true);
    } catch (e: any) {
  ...
  async function addManual() {
    ...
    try {
      const res = await authedFetch("/api/matchmaker/add-fighter", {
        ...
        body: JSON.stringify({ matchmaking_id: matchmakingId, ...form, sportschool: form.gym }),
      });
  ...
  async function controleer() {
    const ids = rowsForCheck.map(aanmeldingId).filter(Boolean);
    const activeUploadId = viewMode === "latest_open" ? latestUploadId : "";

    if (!rowsForCheck.length) {
      setMsg("Er zijn geen nieuwe open aanmeldingen om te controleren.");
      return;
    }
    ...
    try {
      const res = await authedFetch("/api/matchmaker/scrape/start", {
        ...
        body: JSON.stringify({
          ...
          mode: activeUploadId ? "new_upload" : "open_only",
          only_open: true,
          upload_batch_id: activeUploadId || undefined,
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || "Fightpaspoort check mislukt");

      setViewMode("latest_open");
      setMsg(json?.message || "Fightpaspoort check afgerond. Aanmeldingen hebben nu status gescrapt of scrape mislukt.");
      await load(true);
    } catch (e: any) {
  ...
  async function clearUpload(uploadIdValue: string) {
    ...
    const ok = window.confirm("Weet je zeker dat je deze upload wilt verwijderen? Alle aanmeldingen uit deze upload worden ook verwijderd.");
    if (!ok) return;
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json?.ok) throw new Error(json?.error || "Upload verwijderen mislukt");
  ...
  async function deleteFighter(r: Aanmelding) {
    ...
    if (!id) {
      setMsg("Deze vechter heeft geen aanmelding-id en kan niet worden verwijderd.");
      return;
    }
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(json?.error || "Vechter verwijderen mislukt");
  ...
  function startEdit(r: Aanmelding) {
    ...
    if (!id) {
      setMsg("Deze vechter heeft geen aanmelding-id en kan niet worden bewerkt.");
      return;
    }
    ...
    <main style={pageShell}>
      ...
      <section style={chromeFrame}>
        <header style={heroHeader}>
          ...
          <div style={logoWrap}>
            <Image src={LOGO_SRC} alt="FightSupport" width={390} height={92} priority style={logoImg} />
          </div>

          <div style={heroRight}>
            <Link href="/dashboard/matchmaker/matchmaking" className="fs-back-button">
              <ArrowLeft size={17} />
      ...
      <section style={contentFrame}>
        ...
        <div style={statsGrid}>
          ...
          <Stat icon={<Users size={20} />} label="Nieuwste niet gescrapt" value={latestOpenRows.length} />
          <Stat icon={<Users size={20} />} label="Niet gescrapt" value={openRows.length} />
          <Stat icon={<Users size={20} />} label="Gescrapt" value={scrapedRows.length} />
        </div>
        ...
        <section style={accordionGrid}>
          ...
          >
            <div style={uploadLine}>
              <input ref={inputRef} type="file" accept=".xlsx,.xls,.csv" style={fileInput} />
              <button className="fs-orange-btn compact-action" onClick={upload} disabled={busy}>
                <FileUp size={16} />
                Upload bestand
              </button>
            </div>
          ...
          >
            <div style={manualLine}>
              <input placeholder="Discipline" value={form.discipline} onChange={(e) => setForm((f) => ({ ...f, discipline: e.target.value }))} style={input} />
              <input placeholder="Klasse" value={form.klasse} onChange={(e) => setForm((f) => ({ ...f, klasse: e.target.value }))} style={input} />
              <input placeholder="Geslacht" value={form.geslacht} onChange={(e) => setForm((f) => ({ ...f, geslacht: e.target.value }))} style={input} />
              <input placeholder="Naam" value={form.naam} onChange={(e) => setForm((f) => ({ ...f, naam: e.target.value }))} style={input} />
              <input placeholder="Sportschool" value={form.gym} onChange={(e) => setForm((f) => ({ ...f, gym: e.target.value }))} style={input} />
              <input placeholder="VA nummer" value={form.va_nummer} onChange={(e) => setForm((f) => ({ ...f, va_nummer: e.target.value }))} style={input} />
              <input placeholder="Gewicht" value={form.gewicht} onChange={(e) => setForm((f) => ({ ...f, gewicht: e.target.value }))} style={input} />
              <input placeholder="Email trainer" value={form.email} onChange={(e) => setForm((f) => ({ ...f, email: e.target.value }))} style={input} />
              <input placeholder="Telefoon trainer" value={form.telefoon} onChange={(e) => setForm((f) => ({ ...f, telefoon: e.target.value }))} style={input} />

              <button className="fs-silver-btn compact-action" onClick={addManual} disabled={busy}>
                <Plus size={16} />
                Toevoegen
              </button>
            </div>
        ...
        <section style={metalPanelNoPadding}>
          <div style={tableHead}>
            ...
            <button className="fs-silver-btn compact" onClick={() => load()} disabled={busy}>
              <RefreshCw size={16} />
              Vernieuwen
            </button>
          </div>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={table}>
              <thead>
                <tr>
                  {["Upload", "Bestand", "Niet gescrapt", "Gescrapt", "Mislukt", "Actie"].map((h) => (
              ...
              <tbody>
                {uploadTabs.map((u, index) => {
                  ...
                  const countOpen = id ? openRows.filter((r) => uploadId(r) === id).length : 0;
                  const countScraped = id ? scrapedRows.filter((r) => uploadId(r) === id).length : 0;
                  const countFailed = id ? failedRows.filter((r) => uploadId(r) === id).length : 0;
                  return (
                    <tr key={id || index} className="fs-zebra-row">
                      <td style={td}>
                        <b>{index === 0 ? "Nieuwste upload" : `Upload ${uploadTabs.length - index}`}</b>
                        <br />
                        <span style={smallMuted}>{fmt(id)}</span>
                      </td>
                      <td style={td}>{fmt(u.filename || u.bestandsnaam || u.raw_filename)}</td>
                      <td style={td}>{countOpen}</td>
                      <td style={td}>{countScraped}</td>
                      <td style={td}>{countFailed}</td>
                      <td style={td}>
                        {id && (
                          <button className="fs-red-btn compact" onClick={() => clearUpload(id)} disabled={busy}>
                            <Trash2 size={14} />
                            Verwijder upload
                          </button>
                        )}
                ...
                {!uploads.length && (
                  <tr className="fs-zebra-row">
                    <td colSpan={6} style={td}>Nog geen uploads.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <section style={metalPanelNoPadding}>
          <div style={tableHead}>
            <h2 style={tableTitle}>
              {viewMode === "latest_open" && `Nieuwste niet gescrapte upload (${latestOpenRows.length})`}
              {viewMode === "all_open" && `Niet gescrapt of mislukt (${openRows.length})`}
              {viewMode === "scraped" && `Gescrapt (${scrapedRows.length})`}
              {viewMode === "failed" && `Scrape mislukt (${failedRows.length})`}
              {viewMode === "running" && `Controle bezig (${runningRows.length})`}
              {viewMode === "all" && `Alle aanmeldingen (${allRows.length})`}
            </h2>

            <div style={tableActions}>
              <button className={viewMode === "latest_open" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("latest_open")} disabled={busy}>
                Nieuwste niet gescrapt
              </button>
              <button className={viewMode === "all_open" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("all_open")} disabled={busy}>
                Niet gescrapt
              </button>
              <button className={viewMode === "scraped" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("scraped")} disabled={busy}>
                Gescrapt
              </button>
              <button className={viewMode === "failed" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("failed")} disabled={busy}>
                Mislukt
              </button>
              <button className={viewMode === "running" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("running")} disabled={busy}>
                Bezig
              </button>
              <button className={viewMode === "all" ? "fs-orange-btn compact" : "fs-silver-btn compact"} onClick={() => setViewMode("all")} disabled={busy}>
                Alles
              </button>

              <button className="fs-green-btn compact" onClick={controleer} disabled={busy || !rowsForCheck.length || ["scraped", "running", "all"].includes(viewMode)}>
                <Radar size={15} />
                Check {viewMode === "all_open" || viewMode === "failed" ? "niet gescrapt" : "nieuwste upload"}
              </button>

              <Link href={`/dashboard/matchmaker/matchmaking/${matchmakingId}/match`} className="fs-blue-btn compact">
                <Users size={15} />
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={table}>
              <thead>
                <tr>
                  {["Status", "Discipline", "Klasse", "Geslacht", "Naam", "Sportschool", "VA nummer", "Gewicht", "Acties"].map((h) => (
                    <th style={th} key={h}>{h}</th>
                  ))}
                </tr>
              </thead>
              ...
              <tbody>
                {visibleRows.map((r, index) => {
                  const id = aanmeldingId(r);
                  const key = id || `${name(r)}-${pick(r, ["va_nummer", "va"])}-${index}`;
                  const isEditing = Boolean(id && editingId === id && editForm);

                  return (
                    <tr key={key} className="fs-zebra-row">
                      <td style={td}><StatusBadge status={statusOf(r)} scrapedAt={pick(r, ["scraped_at"])} failedAt={pick(r, ["scrape_failed_at"])} /></td>

                      {isEditing && editForm ? (
                        <>
                          <td style={td}><input style={cellInput} value={editForm.discipline} onChange={(e) => setEditForm((f) => f ? { ...f, discipline: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.klasse} onChange={(e) => setEditForm((f) => f ? { ...f, klasse: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.geslacht} onChange={(e) => setEditForm((f) => f ? { ...f, geslacht: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.naam} onChange={(e) => setEditForm((f) => f ? { ...f, naam: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.gym} onChange={(e) => setEditForm((f) => f ? { ...f, gym: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.va_nummer} onChange={(e) => setEditForm((f) => f ? { ...f, va_nummer: e.target.value } : f)} /></td>
                          <td style={td}><input style={cellInput} value={editForm.gewicht} onChange={(e) => setEditForm((f) => f ? { ...f, gewicht: e.target.value } : f)} /></td>
                          <td style={td}>
                            <div style={actionCell}>
                              <button className="fs-green-btn compact" onClick={saveEdit} disabled={busy}>
                                <Save size={14} />
                                Opslaan
                              </button>
                              <button className="fs-silver-btn compact" onClick={() => { setEditingId(null); setEditForm(null); }} disabled={busy}>
                                <X size={14} />
                                Annuleer
                              </button>
                            </div>
                        ...
                        <>
                          <td style={td}>{fmt(pick(r, ["discipline", "sport"]))}</td>
                          <td style={td}>{fmt(pick(r, ["klasse", "klasse_mm"]))}</td>
                          <td style={td}>{fmt(pick(r, ["geslacht", "gender"]))}</td>
                          <td style={td}><b>{name(r)}</b></td>
                          <td style={td}>{fmt(pick(r, ["gym", "sportschool", "sportschool_naam"]))}</td>
                          <td style={td}>{fmt(pick(r, ["va_nummer", "va", "fightpaspoort_nummer"]))}</td>
                          <td style={td}>{fmt(pick(r, ["gewicht", "gewicht_kg"]))}</td>
                          <td style={td}>
                            <div style={actionCell}>
                              {viewMode !== "scraped" && (
                                <>
                                  <button className="fs-silver-btn compact" onClick={() => startEdit(r)} disabled={busy || !id}>
                                    <Edit3 size={14} />
                                    Bewerk
                                  </button>

                                  <button className="fs-red-btn compact" onClick={() => deleteFighter(r)} disabled={busy || !id}>
                                    <Trash2 size={14} />
                                    Verwijder
                                  </button>
                                </>
...
function StatusBadge({ status, scrapedAt, failedAt }: { status: string; scrapedAt?: string; failedAt?: string }) {
  const normalized = normalizeStatus(status);
  const label =
    normalized === "gescrapt" ? "Gescrapt" :
    normalized === "scrape_mislukt" ? "Mislukt" :
    normalized === "controle_bezig" ? "Bezig" :
    normalized === "gematcht" ? "Gematcht" :
    normalized === "afgemeld" ? "Afgemeld" :
    "Rauw";

  const title = normalized === "gescrapt" && scrapedAt
    ? `Gescrapt op ${scrapedAt}`
...
function AccordionPanel({
  ...
  return (
    <div style={metalPanel}>
      <button type="button" style={accordionButton} onClick={() => setOpen(!open)}>
    ...
    <div style={waitOverlay}>
      <div style={waitCard}>
        <div style={waitLogoBox}>
          <Image src={LOGO_SRC} alt="FightSupport" width={330} height={85} style={waitLogoImg} />
        </div>
...
const heroRight: CSSProperties = { display: "flex", justifyContent: "flex-end" };
...
const contentFrame: CSSProperties = {
  ...
  borderRadius: 20,
  padding: 22,
  background: "linear-gradient(135deg,#26282e 0%,#111216 42%,#2f3239 70%,#111216 100%)",
  boxShadow: "inset 0 0 0 1px rgba(255,255,255,.14), 0 24px 60px rgba(0,0,0,.45)",
  color: "#f7f7f7",
};
...
const metalPanel: CSSProperties = {
  ...
  boxShadow: "inset 0 1px 0 rgba(255,255,255,.18), inset 0 -1px 0 rgba(0,0,0,.5), 0 16px 34px rgba(0,0,0,.35)",
  color: "#f7f7f7",
  overflow: "hidden",
};
...
const waitOverlay: CSSProperties = {
  ...
  placeItems: "center",
  padding: 24,
  background: "radial-gradient(circle at center, rgba(255,77,0,.14), rgba(0,0,0,.88) 45%, rgba(0,0,0,.95))",
...
const progressFill: CSSProperties = {
  ...
  width: "42%",
  borderRadius: 999,
  background: "linear-gradient(90deg,transparent,#ff4d00,#fff,#ff4d00,transparent)",
