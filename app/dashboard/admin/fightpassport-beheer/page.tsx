    ...
    {tab === "ai" && <>
      ...
      <section style={{...styles.panel,marginTop:16}}>
        <div style={styles.bulkBar}>
          ...
          {aiView === "open" && <>
            <button style={{...styles.iconAction,...styles.iconDelete}} disabled={!selectedMissingVa.length || bulkMissingBusy || !!missingBusy} onClick={()=>bulkUpdateMissingVa("confirm_deleted")} title="Geselecteerde VA-nummers verwijderen" aria-label="Geselecteerde VA-nummers verwijderen"><Trash2 size={16}/></button>
            <button style={{...styles.iconAction,...styles.iconRetry}} disabled={!selectedMissingVa.length || bulkMissingBusy || !!missingBusy} onClick={()=>bulkUpdateMissingVa("retry")} title="Geselecteerde VA-nummers opnieuw proberen" aria-label="Geselecteerde VA-nummers opnieuw proberen"><RotateCcw size={16}/></button>
            <button style={{...styles.iconAction,...styles.iconResolve}} disabled={!selectedMissingVa.length || bulkMissingBusy || !!missingBusy} onClick={()=>bulkUpdateMissingVa("resolve")} title="Geselecteerde VA-nummers als opgelost markeren" aria-label="Geselecteerde VA-nummers als opgelost markeren"><CheckCircle2 size={17}/></button>
          </>}
        ...
        <tbody>{visibleMissingVa.map((item:any)=>{
          ...
          return <tr key={item.id}>
            ...
            <Td><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              {aiView==="open"&&item.status!=="confirmed_deleted"&&<button style={{...styles.iconAction,...styles.iconDelete}} disabled={!!missingBusy || bulkMissingBusy} onClick={()=>updateMissingVa(item.va_number,"confirm_deleted")} title="Definitief als verwijderd markeren" aria-label={`VA ${vaNumber} definitief als verwijderd markeren`}><Trash2 size={16}/></button>}
              {aiView==="open"&&item.status!=="retry_requested"&&item.status!=="resolved"&&<button style={{...styles.iconAction,...styles.iconRetry}} disabled={!!missingBusy || bulkMissingBusy} onClick={()=>updateMissingVa(item.va_number,"retry")} title="Opnieuw proberen" aria-label={`VA ${vaNumber} opnieuw proberen`}><RotateCcw size={16}/></button>}
              {item.status==="confirmed_deleted"&&<button style={{...styles.iconAction,...styles.iconRetry}} disabled={!!missingBusy || bulkMissingBusy} onClick={()=>updateMissingVa(item.va_number,"restore")} title="Terugzetten naar beoordeling" aria-label={`VA ${vaNumber} terugzetten naar beoordeling`}><RotateCcw size={16}/></button>}
              {aiView==="open"&&item.status!=="resolved"&&<button style={{...styles.iconAction,...styles.iconResolve}} disabled={!!missingBusy || bulkMissingBusy} onClick={()=>updateMissingVa(item.va_number,"resolve")} title="Als opgelost markeren" aria-label={`VA ${vaNumber} als opgelost markeren`}><CheckCircle2 size={17}/></button>}
            </div></Td>
          </tr>})}
...
orange:{display:"inline-flex",alignItems:"center",gap:7,height:40,padding:"0 16px",border:"1px solid #ff7b3b",background:"linear-gradient(#ff6a20,#d83e00)",color:"white",fontWeight:900,cursor:"pointer"},table:{width:"100%",borderCollapse:"collapse",fontSize:13},th:{textAlign:"left",padding:"10px 9px",borderBottom:"1px solid #555",color:"#ff8c58",whiteSpace:"nowrap"},td:{padding:"10px 9px",borderBottom:"1px solid #2d3238",verticalAlign:"top",color:"#f0f0f0"},mini:{background:"#e8e8e8",border:"1px solid #999",color:"#111",fontWeight:800,padding:"6px 10px",cursor:"pointer"},dangerMini:{display:"inline-flex",alignItems:"center",gap:5,background:"#4a1717",border:"1px solid #a94444",color:"#fff",fontWeight:800,padding:"6px 10px",cursor:"pointer"},iconAction:{width:34,height:34,padding:0,display:"inline-flex",alignItems:"center",justifyContent:"center",borderRadius:0,cursor:"pointer",boxShadow:"inset 0 1px 0 rgba(255,255,255,.08),0 3px 8px rgba(0,0,0,.28)"},iconDelete:{border:"1px solid #b4463a",background:"linear-gradient(180deg,#3a1714,#1e0d0b)",color:"#ff7467"},iconRetry:{border:"1px solid #b96a22",background:"linear-gradient(180deg,#38220f,#1f1309)",color:"#ffad55"},iconResolve:{border:"1px solid #3b9a57",background:"linear-gradient(180deg,#17351f,#0d2113)",color:"#7ee39a"},stop:{display:"inline-flex",alignItems:"center",gap:5,background:"#6d2020",border:"1px solid #c94a4a",color:"#fff",fontWeight:900,padding:"6px 10px",cursor:"pointer"},link:{background:"none",border:0,color:"#ff8852",fontWeight:900,cursor:"pointer",padding:0},pagination:{display:"flex",justifyContent:"center",alignItems:"center",gap:12,marginTop:16,flexWrap:"wrap"}};
