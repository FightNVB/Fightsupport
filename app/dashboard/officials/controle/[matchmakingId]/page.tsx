...
function statusFromResultatenOrOk(
  ...
  // De rules-engine is leidend voor de partijstatus:
  // geen melding = OK. Een ontbrekend oud/scrape-veld mag niet zelfstandig
  // een groene partij veranderen in "GEEN INFO".
...
function formatSearchDate(value?: string | null) {
  const raw = normalizeSearchDate(value);
  if (!raw) return "-";
  const [y, m, d] = raw.split("-");
  return y && m && d ? `${d}-${m}-${y}` : raw;
}

function AddBoutFighterSearchField({
  label,
  value,
  onChange,
  onSelect,
  ...
  onSelect: (fighter: AddBoutSearchFighter) => void;
}) {
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<"persoon" | "sportschool">("persoon");
  const [q, setQ] = useState("");
  const [birthDate, setBirthDate] = useState("");
  const [schoolQuery, setSchoolQuery] = useState("");
  const [selectedSchool, setSelectedSchool] = useState<AddBoutSearchSchool | null>(null);
  const [schools, setSchools] = useState<AddBoutSearchSchool[]>([]);
  const [fighters, setFighters] = useState<AddBoutSearchFighter[]>([]);
  ...
  async function ensureSchoolsLoaded() {
    ...
    setSearchError("");
    try {
      const res = await authedFetch("/api/matchmaker/fighter-selector", {
        cache: "no-store",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(json?.error || "Sportscholen konden niet worden geladen.");
      }
      setSchools(Array.isArray(json?.sportscholen) ? json.sportscholen : []);
    } catch (e: any) {
      setSearchError(e?.message || "Sportscholen konden niet worden geladen.");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function searchPerson() {
    if (!q.trim() && !birthDate) {
      setSearchError("Vul naam/VA nummer of geboortedatum in.");
      return;
    }
    setLoading(true);
    setSearchError("");
    ...
    try {
      ...
      const res = await authedFetch(
        `/api/matchmaker/fighter-selector?${params.toString()}`,
        { cache: "no-store" },
      );
      const json = await res.json().catch(() => ({}));
      ...
      setSearchError(e?.message || "Zoeken mislukt.");
    } finally {
      setLoading(false);
    }
  }

  async function chooseSchool(school: AddBoutSearchSchool) {
    ...
    try {
      const res = await authedFetch(
        `/api/matchmaker/fighter-selector?sportschool_id=${encodeURIComponent(
          String(school.sportschool_id),
        )}`,
        { cache: "no-store" },
      );
      const json = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(json?.error || "Vechters konden niet worden geladen.");
      }
      setFighters(Array.isArray(json?.fighters) ? json.fighters : []);
    } catch (e: any) {
      setSearchError(e?.message || "Vechters konden niet worden geladen.");
    } finally {
      setLoading(false);
    }
  }

  function chooseFighter(fighter: AddBoutSearchFighter) {
    onSelect(fighter);
    setOpen(false);
  }
      ...
      <label className="block">
        ...
        <div className="flex items-center gap-2">
          <input
            value={value}
            onChange={(e) => onChange(e.target.value)}
            className="min-w-0 flex-1 rounded-lg px-3 py-2 bg-white text-zinc-900 border border-zinc-300 focus:outline-none focus:border-white/30"
          />
          <button
            type="button"
            onClick={openSearch}
            title="Zoek vechter"
            className="inline-flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-lg border border-zinc-300 bg-zinc-100 text-zinc-900 hover:bg-zinc-200"
          >
            <Search size={17} />
          </button>
        </div>
            ...
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-zinc-400">
                  Zoek op naam/geboortedatum of via sportschool. De sportschool van de partij blijft handmatig.
                </div>
              </div>
              <button
                type="button"
                onClick={() => setOpen(false)}
                className="rounded-md px-3 py-2 text-zinc-400 hover:bg-white/10 hover:text-white"
              >
                ✕
              </button>
            </div>
            ...
            {mode === "persoon" ? (
              <div>
                ...
                <div className="mt-3 flex justify-end">
                  <button
                    type="button"
                    onClick={() => void searchPerson()}
                    disabled={loading}
                    className="inline-flex items-center gap-2 rounded-md bg-orange-600 px-4 py-2 text-sm font-extrabold text-white hover:bg-orange-500 disabled:opacity-50"
                  >
                    <Search size={15} />
              ...
              <div>
                <label className="relative block text-xs font-bold text-zinc-300">
                  ...
                  <input
                    value={schoolQuery}
                    onFocus={() => void ensureSchoolsLoaded()}
                    onChange={(e) => {
                  ...
                  filteredSchools.length > 0 ? (
                    <div className="absolute z-[1102] mt-1 max-h-56 w-full overflow-auto rounded-md border border-white/15 bg-zinc-900 shadow-xl">
                      {filteredSchools.map((school) => (
                        <button
                          key={school.sportschool_id}
                          type="button"
                          onClick={() => void chooseSchool(school)}
                ...
                {selectedSchool ? (
                  <div className="mt-3 rounded-md border border-orange-500/25 bg-orange-500/10 px-3 py-2 text-sm">
                    <b>{selectedSchool.naam}</b>
                    {selectedSchool.plaats ? ` · ${selectedSchool.plaats}` : ""}
                    <span className="ml-2 text-xs text-zinc-400">
                      — kies hieronder een vechter
                    </span>
                  </div>
                ) : null}
              </div>
            ...
            <div className="mt-4 max-h-[45vh] overflow-auto rounded-lg border border-white/10">
              {loading ? (
                ...
                <div className="p-5 text-center text-sm text-zinc-500">
                  {mode === "sportschool" && !selectedSchool
                    ? "Zoek en selecteer eerst een sportschool."
                    : "Nog geen resultaten."}
                </div>
              ) : (
                fighters.map((fighter) => (
                  <div
                    key={`${fighter.va_nummer}-${fighter.naam}`}
                  ...
                  >
                    <div>
                      ...
                      <div className="mt-0.5 text-xs text-zinc-400">
                        VA {fighter.va_nummer || "-"} ·{" "}
                        {formatSearchDate(fighter.geboortedatum)}
                      </div>
                    </div>
                    <button
                      type="button"
...
export default function ControleMatchmakingPage() {
  ...
  const [fRoodGeboortedatum, setFRoodGeboortedatum] = useState("");
  ...
  const [fBlauwGeboortedatum, setFBlauwGeboortedatum] = useState("");
  ...
  function blockReadOnlyInteraction(event: React.SyntheticEvent) {
    ...
    const names = otherPresenceUsers.map((u) => safeText(u.user_name, "een andere gebruiker")).join(", ");
    setMsg(names ? `🔒 Alleen-lezen: ${names} heeft deze matchmaking al geopend.` : "🔒 Alleen-lezen: er is geen bewerk-lock voor deze sessie. Open de matchmaking opnieuw.");
  }
  ...
  async function addPartijSubmit() {
    ...
    try {
      ...
      setFRoodGym("");
      setFRoodVa("");
      setFRoodGeboortedatum("");
      setFRoodKg("");
      setFBlauwNaam("");
      setFBlauwGym("");
      setFBlauwVa("");
      setFBlauwGeboortedatum("");
      setFBlauwKg("");
      setFMaxKg("");
      setReloadTick((x) => x + 1);
    } catch (e: any) {
  ...
  async function loadDuurGoedkeuring(berekendeMinuten?: number | null) {
    ...
    try {
      const query = berekendeMinuten != null
      ...
      console.warn("[officials page] duurgoedkeuring ophalen mislukt:", e);
  ...
  async function load() {
    ...
    try {
      ...
      const liveChecks = await loadActueleFighterChecks(matchmakingId, latestControleRunId ?? "", ctxList.flatMap((r: AnyRow) => [liveVa(r, "rood"), liveVa(r, "blauw")]));
      ctxList=ctxList.map((r:AnyRow)=>mergeLive(mergeLive(r,"rood",liveChecks.get(liveVa(r,"rood"))),"blauw",liveChecks.get(liveVa(r,"blauw"))));
      setRows(ctxList);
  ...
  useEffect(() => {
    ...
    // Browser terug/vooruit kan deze pagina uit de bfcache herstellen.
    // Vraag dan opnieuw aan de server wie de bewerk-lock heeft.
    const onPageShow = () => {
      void claimPresence();
    };
  ...
  const galaDuur = useMemo(() => {
    if (galaDuurCalc)
      return buildGalaDuurFromMins(galaDuurCalc.totalMins, aantalUren, evenementDatum);
    return null;
  ...
  useEffect(() => {
    ...
    // Ook zonder ingestelde 7/8/9 uur moet de goedkeuringsstatus worden geladen.
    // Boven 9,5 uur is altijd expliciete goedkeuring vereist.
  ...
  return (
    <Shell>
      <div style={metalFrameStyle("orange")} className="p-3 md:p-4" onClickCapture={blockReadOnlyInteraction} onSubmitCapture={blockReadOnlyInteraction}>
                    ...
                    >
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-zinc-800 font-black">{formatDurationExact(galaDuur.mins)}</span>
                          {aantalUren != null ? (
                            <span className="rounded-full bg-zinc-900 px-2.5 py-1 text-[11px] font-black text-white">Ingesteld: {aantalUren} uur</span>
                          ) : (
                            <span className="rounded-full bg-red-600 px-2.5 py-1 text-[11px] font-black text-white">Uren niet ingesteld</span>
                          )}
                          {galaDuur.needsApproval ? (
                            <span className="rounded-full bg-red-600 px-2.5 py-1 text-[11px] font-black text-white">GOEDKEURING NODIG</span>
                          ) : (
                            <span className="rounded-full bg-green-600 px-2.5 py-1 text-[11px] font-black text-white">BINNEN NORM</span>
                          )}
                          {duurGoedkeuring?.goedkeuring_geldig ? (
                            <span className="rounded-full bg-teal-600 px-2.5 py-1 text-[11px] font-black text-white">GOEDGEKEURD</span>
                          ) : null}
                        </div>
                      </div>
                      ...
                      <div className="border-t border-zinc-300 px-3 py-3 text-sm text-zinc-800">
                        ...
                        {galaDuur?.basisMin != null ? (
                          <div className="mt-2 text-xs text-zinc-600">
                            Toegestaan nu: <span className="font-black">{galaDuur.toegestaneMinuten != null ? formatDurationExact(galaDuur.toegestaneMinuten) : "-"}</span>{" "}
                            ({aantalUren} uur + {galaDuur.margeMinuten} min marge).
                            {galaDuur.overschrijdingMinuten != null && galaDuur.overschrijdingMinuten > 0 ? (
                              <> Overschrijding boven basis: <span className="font-black text-red-700">+{formatDurationExact(galaDuur.overschrijdingMinuten)}</span>.</>
                            ) : null}
                          </div>
                        ) : null}

                        {galaDuur?.needsApproval ? (
                          <div className="mt-3 rounded-xl border-2 p-3" style={{
                            borderColor: duurGoedkeuring?.goedkeuring_geldig ? "rgba(13,148,136,0.55)" : "rgba(220,38,38,0.45)",
                            background: duurGoedkeuring?.goedkeuring_geldig ? "rgba(204,251,241,0.55)" : "rgba(254,226,226,0.65)",
                          }}>
                            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                              <div>
                                <div className="font-black text-zinc-900">
                                  {duurGoedkeuring?.goedkeuring_geldig ? "Deze overschrijding is goedgekeurd." : "Goedkeuring van de galaduur vereist."}
                                </div>
                                <div className="mt-1 text-xs text-zinc-700">
                                  ...
                                  {duurGoedkeuring?.latest_approval?.approved_at ? (
                                    <> Laatste goedkeuring: {new Date(duurGoedkeuring.latest_approval.approved_at).toLocaleString("nl-NL")}.</>
                                  ) : null}
                                </div>
                              </div>
                              {canApproveDuur ? (
                                <div className="flex flex-wrap gap-2 shrink-0">
                                  {!duurGoedkeuring?.goedkeuring_geldig ? (
                                    <DarkActionButton label={duurGoedkeuringBusy ? "Bezig..." : "Goedkeuren"} tone="green" icon={<Check className="h-3.5 w-3.5" />} onClick={() => wijzigDuurGoedkeuring("approve")} disabled={duurGoedkeuringBusy || duurGoedkeuringLoading} />
                                  ) : (
                                    <DarkActionButton label={duurGoedkeuringBusy ? "Bezig..." : "Goedkeuring intrekken"} tone="red" icon={<X className="h-3.5 w-3.5" />} onClick={() => wijzigDuurGoedkeuring("revoke")} disabled={duurGoedkeuringBusy || duurGoedkeuringLoading} />
                                  )}
                                </div>
                              ) : null}
                            </div>
                                  ...
                                  {groep.deelnemers.map((deelnemer) => (
                                    ...
                                    >
                                      <div className="flex flex-wrap items-start justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                          <Link
                                            href={`/dashboard/officials/controle/${matchmakingId}/fighter/${deelnemer.va}`}
                    ...
                    <tbody>
                      {filteredRows.map((r, i) => {
                        ...
                        const roodDetailHref =
                          roodVA && roodVA !== "-"
                            ? `/dashboard/officials/controle/${encodeURIComponent(
                                matchmakingId,
                            ...
                            <td className="py-3 px-4 align-top">
                              <div className="flex flex-wrap gap-2">
                                <Link
                                  href={`/dashboard/officials/controle/${encodeURIComponent(
                                    matchmakingId,
                                  )}/partij/${encodeURIComponent(String(originalPn))}`}
                                  className="px-3 py-1.5 rounded font-extrabold text-sm"
                                  style={{
                                    background: "rgba(0,0,0,0.55)",
                                    border: `1px solid rgba(255,77,0,0.85)`,
                ...
                <div className="px-6 py-5 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="rounded-xl border-2 border-zinc-300 bg-white p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <AddBoutFighterSearchField
                          label="Naam rood"
                          value={fRoodNaam}
                          onChange={setFRoodNaam}
                          onSelect={(fighter) => {
                            setFRoodNaam(fighter.naam);
                            setFRoodVa(fighter.va_nummer);
                            setFRoodGeboortedatum(
                              normalizeSearchDate(fighter.geboortedatum),
                            );
                          }}
                        />
                        <Field
                        ...
                        />
                        <Field
                          label="Geboortedatum rood"
                          value={fRoodGeboortedatum}
                          onChange={setFRoodGeboortedatum}
                          type="date"
                        />
                        <Field
                          label="KG rood"
                    ...
                    <div className="rounded-xl border-2 border-zinc-300 bg-white p-4">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <AddBoutFighterSearchField
                          ...
                          onSelect={(fighter) => {
                            setFBlauwNaam(fighter.naam);
                            setFBlauwVa(fighter.va_nummer);
                            setFBlauwGeboortedatum(
                              normalizeSearchDate(fighter.geboortedatum),
                            );
                          }}
                        ...
                        />
                        <Field
                          label="Geboortedatum blauw"
                          value={fBlauwGeboortedatum}
                          onChange={setFBlauwGeboortedatum}
                          type="date"
                        />
                        <Field
                          label="KG blauw"
