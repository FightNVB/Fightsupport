...
function normRoleName(v: any): string {
  return String(v ?? "")
    .trim()
    .toLowerCase()
    .replace(/[\s_-]+/g, "");
...
function isVerbodRule(row: Partial<ControleResultaatRow> | null | undefined): boolean {
  const hay = [
    row?.rule_code,
    row?.rule,
    row?.boodschap,
...
export default function PartijDetailPage() {
  ...
  const normalizedRoles = useMemo(() => roleNames.map(normRoleName), [roleNames]);
  const isSuperadmin = useMemo(() => normalizedRoles.includes("superadmin"), [normalizedRoles]);
  const isAdmin = useMemo(() => normalizedRoles.includes("admin") || normalizedRoles.includes("superadmin"), [normalizedRoles]);
  const isHoofdofficial = useMemo(
    () => normalizedRoles.includes("hoofdofficial") || normalizedRoles.includes("headofficial"),
    [normalizedRoles]
  );
  const isOfficial = useMemo(
    () => normalizedRoles.includes("official") || normalizedRoles.includes("officials"),
    [normalizedRoles]
  );
  const isMatchmaker = useMemo(() => normalizedRoles.includes("matchmaker"), [normalizedRoles]);
  ...
  async function loadMyRoles() {
    ...
    // 1) Nieuwe rollenstructuur: user_roles -> roles
    ...
    if (roleIds.length > 0) {
      ...
      if (rrErr) {
        ...
        names.push(...(rr ?? []).map((r: any) => String(r?.name ?? "").trim()).filter(Boolean));
      }
    }

    // 2) Fallback/oudere structuur: public.user_profiles.role
    // Dit voorkomt dat officials geen knoppen zien als hun rol niet in user_roles staat.
    ...
    if (profErr) {
      console.error("Fout bij laden user_profiles.role:", profErr);
    } else if ((profile as any)?.role) {
      names.push(String((profile as any).role).trim());
    }

    const uniqueNames = Array.from(new Set(names.filter(Boolean)));
    setRoleNames(uniqueNames);
    return { uid, roles: uniqueNames };
  }
  ...
  function canReviewDecision(r: ControleResultaatRow, decision: "approve" | "reject") {
    ...
    // Dispensatie direct reviewen: alleen superadmin.
    if (res === "dispensatie") return isSuperadmin;

    const isInfoOfActie = res === "info" || res === "actie";
    const isAfkeur = res === "afgekeurd";

    // Superadmin: alles.
    if (isSuperadmin) return true;

    // Hoofdofficial + admin: INFO/ACTIE/AFKEUR goed- of afkeuren.
    if (isHoofdofficial || isAdmin) return isInfoOfActie || isAfkeur;

    // Matchmaker + official: alleen INFO/ACTIE goedkeuren, niet afkeuren.
    if (isMatchmaker || isOfficial) {
      return decision === "approve" && isInfoOfActie;
    }
  ...
  function canApproveRule(r: ControleResultaatRow) {
    return canReviewDecision(r, "approve") || canReviewDecision(r, "reject");
  }
  ...
  async function approveSingle(resultaatId: string) {
    ...
    try {
      const row = regels.find((r) => r.id === resultaatId);
      if (row && !canReviewDecision(row, "approve")) {
  ...
  async function rejectSingle(resultaatId: string) {
    ...
    try {
      const row = regels.find((r) => r.id === resultaatId);
      if (row && !canReviewDecision(row, "reject")) {
                  ...
                  <table className="w-full text-sm border-collapse">
                    ...
                    <tbody className="[&>tr:nth-child(odd)]:bg-white [&>tr:nth-child(odd)]:text-zinc-900 [&>tr:nth-child(even)]:bg-zinc-700 [&>tr:nth-child(even)]:text-white">
                      {regels.map((r, idx) => {
                        ...
                        const canApprove = !isWeegstation && canReviewDecision(r, "approve");
                        const canReject = !isWeegstation && canReviewDecision(r, "reject");
