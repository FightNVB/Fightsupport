...
type UitslagenRow = {
  ...
  totalPartijen: number;
  ingevuldUitslagen: number;
};
...
function UitslagenStatusBadge({
  ...
  const klaar = total > 0 && ingevuld >= total;
  if (klaar) {
    return (
      <span
...
export default function OfficialsUitslagenOverzichtPage() {
  const router = useRouter();
  const { user, roles, loading: authLoading } = useAuth();
  const [rows, setRows] = useState<UitslagenRow[]>([]);
  ...
  async function load() {
    ...
    try {
      const [
        ...
        { data: bouts, error: boutsError },
        { data: uitslagen, error: uitslagenError },
      ] = await Promise.all([
        supabase
      ...
      const uitslCountByMm: Record<string, number> = {};
      for (const u of uitslagen ?? []) {
        const id = u.matchmaking_id as string;
        uitslCountByMm[id] = (uitslCountByMm[id] ?? 0) + 1;
      }

      const rows: UitslagenRow[] = (uploads ?? []).map((upload) => ({
        ...
        totalPartijen: partijCountByMm[upload.matchmaking_id as string] ?? 0,
        ingevuldUitslagen: uitslCountByMm[upload.matchmaking_id as string] ?? 0,
      }));

      setRows(rows);
    } catch (e: any) {
      console.error("Fout bij laden uitslagen overzicht:", e);
                ...
                <div className="px-2 md:px-3 py-2">
                  <div className="mt-2 text-center">
                    ...
                    <p className="mt-2 text-sm md:text-base text-zinc-700">
                      Selecteer een evenement om uitslagen in te voeren
                    </p>
                  </div>
                        ...
                        <table className="min-w-full border-collapse">
                          ...
                          >
                            <tr>
                              ...
                              <th className="py-3 px-4 text-left text-sm font-semibold">Ingevuld</th>
                                ...
                                >
                                  ...
                                  <td className="py-3 px-4">
                                    <span
                                      ...
                                      style={{
                                        background:
                                          row.ingevuldUitslagen >= row.totalPartijen && row.totalPartijen > 0
                                            ? "rgba(34,197,94,0.14)"
                                            : "rgba(255,193,7,0.16)",
                                        color:
                                          row.ingevuldUitslagen >= row.totalPartijen && row.totalPartijen > 0
                                    ...
                                    >
                                      {row.ingevuldUitslagen}/{row.totalPartijen}
                                    </span>
                                  </td>
                                  <td className="py-3 px-4">
                                    <UitslagenStatusBadge
                                      ingevuld={row.ingevuldUitslagen}
                                      total={row.totalPartijen}
                                    />
                                  </td>
                                  <td className="py-3 px-4">
                                    <Link
