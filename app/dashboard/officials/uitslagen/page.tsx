...
export default function OfficialsUitslagenPage() {
  ...
  async function load() {
    ...
    try {
      const [{ data: lineupRows, error: lineErr }, { data: uitslagenRows, error: uitsErr }] =
        await Promise.all([
          supabase
            .from("lineup_bouts")
            .select("matchmaking_id, bondteam, evenement_naam, evenement_datum"),
          supabase
            .from("uitslagen_lineup")
            .select("matchmaking_id, finalized"),
        ]);
      ...
      for (const row of uitslagenRows ?? []) {
        ...
        if (!cur) continue;
        cur.ingevuld += 1;
        if (row.finalized === true) cur.finalized = true;
      }

      setRows(Array.from(map.values()).sort((a, b) => String(b.evenement_datum ?? "").localeCompare(String(a.evenement_datum ?? ""))));
    } catch (e: any) {
  ...
  const filtered = rows.filter((row) =>
    [row.evenement_naam, row.evenement_datum, row.bondteam, row.matchmaking_id]
          ...
          <div className="overflow-hidden rounded-[26px]" style={{ background: "linear-gradient(180deg, rgba(41,41,46,0.98) 0%, rgba(24,24,28,0.98) 100%)", border: "2px solid rgba(70,70,74,0.38)" }}>
            <div className="grid grid-cols-1 gap-4 px-5 py-4 md:grid-cols-[1fr_auto_1fr] md:items-center" style={{ borderBottom: "3px solid rgba(255,77,0,0.58)" }}>
              <div className="flex flex-wrap items-center gap-2 md:justify-self-start">
                <NvbLightButton label="← Menu" onClick={() => router.push("/dashboard/officials")} />
              ...
              <div className="text-right md:justify-self-end">
                ...
                <div className="text-[12px] text-white/60">Compact invoeren en finaliseren</div>
              </div>
            </div>
                ...
                <div className="rounded-[20px] p-4 md:p-5" style={contentPanelStyle()}>
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      <p className="text-[13px] text-zinc-600">Selecteer een lineup snapshot om uitslagen te verwerken.</p>
                    </div>
                    ...
                    <div className="relative w-full md:w-[360px]">
                      ...
                      <input
                        ...
                        placeholder="Zoek op event, bondteam of id"
                        className="w-full rounded-2xl border bg-white py-2.5 pl-9 pr-3 text-sm outline-none"
                  ...
                  <div className="mt-4 overflow-hidden rounded-[18px] border border-zinc-500/30">
                    <div className="grid grid-cols-[1.7fr_120px_120px_110px_110px_120px] border-b text-[11px] font-extrabold uppercase tracking-[0.08em] text-white" style={{ background: "linear-gradient(180deg, rgba(56,56,61,1) 0%, rgba(34,34,39,1) 100%)" }}>
                      ...
                      <div className="px-3 py-3">Actie</div>
                    </div>

                    {loading ? (
                      <div className="bg-white px-4 py-8 text-center text-sm text-zinc-500">Laden…</div>
                    ) : filtered.length === 0 ? (
                      <div className="bg-white px-4 py-8 text-center text-sm text-zinc-500">Geen uitslagen gevonden.</div>
                    ) : (
                      filtered.map((row, i) => {
                        const zebra = i % 2 === 0;
                        return (
                          <div
                            ...
                            className="grid grid-cols-[1.7fr_120px_120px_110px_110px_120px] items-center text-[13px]"
                          ...
                          >
                            ...
                            <div className="px-3 py-3 font-bold">{row.ingevuld}</div>
                            <div className="px-3 py-3">
                              <Link
                                href={`/dashboard/officials/uitslagen/${encodeURIComponent(row.matchmaking_id)}`}
